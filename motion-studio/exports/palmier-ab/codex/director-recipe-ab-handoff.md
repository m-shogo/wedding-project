# StaRt A/B Handoff — Codex CLI/agent lane

Palmier / project name for this lane: `START_AB_CODEX`

研究トラック（StaRt Extended Opening, Phase G）。Opening V1の正本ではない。

**このファイルは Codex CLI/agent 専用レーン。もう一方のレーンの出力ディレクトリは読まない・参照しないこと。**
...
1. 実際に render/export した artifact のrepo相対パスを控える（例: `out/palmier-ab/codex/chorus1_ab.mp4`）。
