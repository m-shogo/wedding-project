...
for (const path of [
  ...
  'index.html',
  'main.ts',
  'studio-sandbox/src/Root.tsx',
  'studio-sandbox/src/index.ts',
  'payloads/type-mask-reveal.json',
  'payloads/type-char-stagger.json',
  'payloads/type-type-on-rhythm.json',
]) {
...
if (errors.length === 0) {
  ...
  if (manifest.candidates?.length !== 3) fail('batch must contain exactly three Typography Element candidates');

  const expectedIds = ['type-mask-reveal', 'type-char-stagger', 'type-type-on-rhythm'];
  ...
  for (const token of [
    ...
    'awaiting-confirmation is transport only, not install PASS',
  ]) {
  ...
  for (const token of [
    'TypographyElementActualSandbox',
    'width={1280}',
    'height={720}',
    'fps={30}',
...
console.log('✅ Typography Element Studio Actual batch is prepared, bounded, three-candidate, and honestly NOT_RUN.');
console.log('candidateCount=3');
