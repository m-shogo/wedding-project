...
const pr=run('scripts/profile-v1-production-preflight.mts',['--json']); let productionPreflightReport:any=null; let assemblyReport:any=null; let assembly:Stage;
if(pr.status!==0){assembly={state:'BLOCKED',detail:'Full production preflight failed.',blockers:[...lines(pr.stdout),...lines(pr.stderr)]};}else{try{productionPreflightReport=JSON.parse(pr.stdout)}catch{}assemblyReport=productionPreflightReport?.assembly?.report??null;const ready=productionPreflightReport?.readiness?.productionPreflightReady===true;assembly={state:ready?'PASS':'BLOCKED',detail:ready?'Generated accents + 17 media + BGM rights + structure + real-media Human QA are ready.':'Generated accent contracts or assembly inputs/Human QA are not all ready.',blockers:ready?[]:(productionPreflightReport?.readiness?.blockers??[])};}
const finalRender:Stage=assembly.state!=='PASS'?{state:'NOT_RUN',detail:'Blocked by full production preflight readiness.',path:rel(finalPath)}:!existsSync(finalPath)?{state:'MISSING',detail:'Render the current ProfileV1 production candidate.',path:rel(finalPath)}:(()=>{const r=run('scripts/check-profile-render.mts',['out/profile/profile_v1.mp4']);return r.status===0?{state:'PASS',detail:'Final render passes technical media QA.',path:rel(finalPath)}:{state:'BLOCKED',detail:'Final render technical QA failed.',path:rel(finalPath),blockers:[...lines(r.stdout),...lines(r.stderr)]};})();
...
else if(finalReview.state!=='PASS'){overallState='HUMAN_FINAL_RENDER_REVIEW_REQUIRED_OR_STALE';nextActions=['current final renderに対するHuman reviewを完了','pnpm profile:final-render-review:strict'];}
else if(bundle.state==='MISSING'){overallState='PRODUCTION_BUNDLE_REQUIRED';nextActions=['pnpm export:profile-v1-production-bundle'];}
else if(bundle.state!=='PASS'){overallState='PRODUCTION_BUNDLE_STALE';nextActions=['current artifactsを確認','pnpm export:profile-v1-production-bundle'];}
else if(davinci.state==='MISSING'){overallState='DAVINCI_EVIDENCE_INIT_REQUIRED';nextActions=['pnpm profile:davinci-finishing:init','Mac DaVinci Resolveで実Actualを実施','pnpm profile:davinci-finishing:strict'];}
else if(davinci.state!=='PASS'){overallState='DAVINCI_ACTUAL_REQUIRED_OR_STALE';nextActions=['Mac DaVinci Actualの未完了/FAIL/stale項目を解消','pnpm profile:davinci-finishing:strict'];}
else if(approval.state==='MISSING'){overallState='FINAL_DELIVERY_APPROVAL_INIT_REQUIRED';nextActions=['pnpm profile:final-delivery-approval:init','DaVinci final exportを人間が最終確認','pnpm profile:final-delivery-approval:strict'];}
