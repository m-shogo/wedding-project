...
try {
  ...
  const openingPlan = buildBgmIntakePlan({project: 'opening', sourcePath: openingSource});
  ...
  const existingPlan = buildBgmIntakePlan({project: 'opening', sourcePath: openingSource});
  ...
  const profilePlan = buildBgmIntakePlan({project: 'profile', sourcePath: profileSource});
  ...
  if (!existsSync(profileSource)) throw new Error('Profile BGM source modified');

  const unsupportedPlan = buildBgmIntakePlan({project: 'profile', sourcePath: unsupportedSource, overwrite: true});
  ...
  const missingPlan = buildBgmIntakePlan({project: 'opening', sourcePath: join(root, 'missing.mp3'), overwrite: true});
