...
try {
  ...
  const effectiveCanaryStateAtPreparation =
    !blocked && canary.state === 'BLOCKED_INPUT' ? 'PENDING_RUNTIME' : canary.state;
  const stateNormalizedFromPreparedInput = effectiveCanaryStateAtPreparation !== canary.state;
  ...
  const session = resolveCanarySessionSchema.parse({
    ...
    canaryStateAtPreparation: effectiveCanaryStateAtPreparation,
  ...
  console.log(`   effectiveCanaryState=${effectiveCanaryStateAtPreparation}`);
