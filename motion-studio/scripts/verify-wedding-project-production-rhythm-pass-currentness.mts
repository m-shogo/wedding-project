...
function sceneIdentity(scene:any){return JSON.stringify({sceneId:scene?.sceneId,sourceRevision:scene?.sourceRevision,productionRole:scene?.productionRole??null,patternId:scene?.patternId??null,timingRevision:scene?.timing?.revision??scene?.timingRevision??null,durationFrames:scene?.durationFrames,startFrame:scene?.startFrame,endFrameExclusive:scene?.endFrameExclusive,transitionInFrames:scene?.transitionInFrames??scene?.transitionIn?.durationFrames??0,transitionOutFrames:scene?.transitionOutFrames??scene?.transitionOut?.durationFrames??0});}
