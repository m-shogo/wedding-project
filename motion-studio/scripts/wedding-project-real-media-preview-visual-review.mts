...
const queue={schemaVersion:"wedding-movie-real-media-visual-correction-queue/v1",authority:"DERIVED_ONLY_FROM_EXPLICIT_HUMAN_REAL_MEDIA_VISUAL_FAIL_VERDICTS",generatedAt:new Date().toISOString(),projectId:stills.projectId,source:{stillsManifestPath:stillsPath,stillsManifestSha256:identity.stillsManifestSha256,humanReviewPath:reviewPath,humanReviewSha256:sha256(reviewPath),identityFingerprint},failures,summary:{failedChecks:failures.length,notRunChecks:notRun,finalVerdict,reviewComplete:notRun===0&&finalVerdict!=="NOT_RUN",correctionRequired:failures.length>0,productionReady:false},nextActions:failures.length?{fixFailedScenes:true,refreshFreshVisualQa:refreshCommand,rerenderRealPreview:`node --no-warnings motion-studio/scripts/render-wedding-project-real-media-preview.mts --selected-manifest=\"$HOME/Downloads/${stills.projectId}-selected-scene-render-manifest.json\" --readiness-audit=\"$HOME/Downloads/${stills.projectId}-production-readiness-audit.json\" --output=\"$HOME/Downloads/${stills.projectId}-real-media-preview.mp4\" --manifest=\"$HOME/Downloads/${stills.projectId}-real-media-preview-manifest.json\" --render`,extractFreshQaStills:`node --no-warnings motion-studio/scripts/wedding-project-real-media-preview-qa-stills.mts --preview-manifest=\"$HOME/Downloads/${stills.projectId}-real-media-preview-manifest.json\"`,initFreshVisualReview:`node --no-warnings motion-studio/scripts/wedding-project-real-media-preview-visual-review.mts --stills-manifest=\"$HOME/Downloads/${stills.projectId}-real-media-preview-qa-stills-manifest.json\" --init`}:null,evidenceBoundary:{humanVisualReviewPerformed:review.evidenceBoundary?.humanVisualReviewPerformed===true,remotionStudioGuiActual:"NOT_RUN",palmierGuiActual:"NOT_RUN",macDaVinciGuiActual:"NOT_RUN",productionReady:false,rule:"Only explicit Human FAIL verdicts become correction work. Mechanical still extraction, CLI render, and CI fixtures never create Human visual PASS or GUI Actual evidence."}};
