...
if(stills.summary?.sourceCurrentVerified!==true||stills.summary?.framingCurrentVerified!==true||stills.summary?.humanVisualQa!=="NOT_RUN"||stills.summary?.productionReady!==false)fail("REAL_PREVIEW_VISUAL_REVIEW_STILLS_NOT_CURRENT");
...
const sceneRows=stills.stills??[];
for(const row of sceneRows){if(!row.framingRevision||row.framing?.revision!==row.framingRevision)fail("REAL_PREVIEW_VISUAL_REVIEW_FRAMING_IDENTITY_MISSING",String(row.sceneId));}
const scenes=[...new Map(sceneRows.map((s:any)=>[s.sceneId,{sceneId:s.sceneId,sourceRevision:s.sourceRevision,patternId:s.patternId,productionRole:s.productionRole,framingRevision:s.framingRevision,framing:s.framing}])).values()] as any[];
for(const scene of scenes){const rows=sceneRows.filter((row:any)=>row.sceneId===scene.sceneId);if(rows.some((row:any)=>row.framingRevision!==scene.framingRevision))fail("REAL_PREVIEW_VISUAL_REVIEW_FRAMING_REVISION_MISMATCH",scene.sceneId);}
...
const transitions=(stills.transitionStills??[]).map((t:any)=>{
 ...
 if(t.fromFramingRevision!==from.framingRevision||t.toFramingRevision!==to.framingRevision)fail("REAL_PREVIEW_VISUAL_REVIEW_TRANSITION_FRAMING_STALE",`${t.fromSceneId}->${t.toSceneId}`);
 return {fromSceneId:t.fromSceneId,toSceneId:t.toSceneId,fromSourceRevision:from.sourceRevision,toSourceRevision:to.sourceRevision,fromFramingRevision:from.framingRevision,toFramingRevision:to.framingRevision,durationFrames:t.durationFrames,frame:t.frame,stillSha256:t.sha256};
});
...
if(flag("init")){
 const review={schemaVersion:"wedding-movie-real-media-human-visual-review/v1",authority:"EXPLICIT_HUMAN_REVIEW_OF_CURRENT_REAL_MEDIA_PREVIEW_QA_STILLS",generatedAt:new Date().toISOString(),projectId:stills.projectId,source:{stillsManifestPath:stillsPath,stillsManifestSha256:identity.stillsManifestSha256,previewManifestSha256:identity.previewManifestSha256,previewSha256:identity.previewSha256,identityFingerprint},instructions:"Inspect the extracted Scene and transition stills and play the bound real-media preview when judging tempo. Framing revision is part of Scene identity: after crop/focus changes, regenerate stills and restart review from NOT_RUN. Change only verdict/notes/reviewer/reviewedAt and humanVisualReviewPerformed after actual Human review.",scenes:scenes.map((s:any)=>({...s,checks:Object.fromEntries(SCENE_CHECKS.map((c)=>[c,{verdict:"NOT_RUN" as Verdict,notes:""}]))})),transitions:transitions.map((t:any)=>({...t,checks:Object.fromEntries(TRANSITION_CHECKS.map((c)=>[c,{verdict:"NOT_RUN" as Verdict,notes:""}]))})),finalVerdict:"NOT_RUN" as Verdict,finalNotes:"",reviewer:null,reviewedAt:null,evidenceBoundary:{humanVisualReviewPerformed:false,remotionStudioGuiActual:"NOT_RUN",palmierGuiActual:"NOT_RUN",macDaVinciGuiActual:"NOT_RUN",productionReady:false}};
...
review.scenes.forEach((scene:any,i:number)=>{const expected:any=scenes[i];for(const k of ["sceneId","sourceRevision","patternId","productionRole","framingRevision"]){if(scene[k]!==expected[k])fail("REAL_PREVIEW_VISUAL_REVIEW_SCENE_IDENTITY_MISMATCH",`${expected.sceneId}:${k}`);}if(fingerprint(scene.framing)!==fingerprint(expected.framing))fail("REAL_PREVIEW_VISUAL_REVIEW_SCENE_FRAMING_MISMATCH",expected.sceneId);for(const c of SCENE_CHECKS){const v=checkVerdict(scene.checks?.[c]?.verdict,`${scene.sceneId}:${c}`);if(v==="FAIL")failures.push({kind:"SCENE",sceneId:scene.sceneId,sourceRevision:scene.sourceRevision,framingRevision:scene.framingRevision,patternId:scene.patternId,productionRole:scene.productionRole,check:c,notes:String(scene.checks?.[c]?.notes??""),returnTo:c==="CROP_SUBJECT_SAFE"?"REAL_MEDIA_CROP":c==="VISUAL_TEMPO_FEELS_INTENTIONAL"?"SCENE_TIMING_AND_RHYTHM":"SCENE_BOUND_A_B_COMPARE"});}});
...
review.transitions.forEach((tr:any,i:number)=>{const e:any=transitions[i];for(const k of ["fromSceneId","toSceneId","fromSourceRevision","toSourceRevision","fromFramingRevision","toFramingRevision","durationFrames","frame","stillSha256"]){if(tr[k]!==e[k])fail("REAL_PREVIEW_VISUAL_REVIEW_TRANSITION_IDENTITY_MISMATCH",`${e.fromSceneId}->${e.toSceneId}:${k}`);}for(const c of TRANSITION_CHECKS){const v=checkVerdict(tr.checks?.[c]?.verdict,`${tr.fromSceneId}->${tr.toSceneId}:${c}`);if(v==="FAIL")failures.push({kind:"TRANSITION",fromSceneId:tr.fromSceneId,toSceneId:tr.toSceneId,fromSourceRevision:tr.fromSourceRevision,toSourceRevision:tr.toSourceRevision,fromFramingRevision:tr.fromFramingRevision,toFramingRevision:tr.toFramingRevision,durationFrames:tr.durationFrames,check:c,notes:String(tr.checks?.[c]?.notes??""),returnTo:"SCENE_EDGE_TRANSITION"});}});
...
const refreshCommand=`node --no-warnings motion-studio/scripts/refresh-wedding-project-real-media-visual-qa.mts --selected-manifest=\"$HOME/Downloads/${stills.projectId}-selected-scene-render-manifest.json\" --readiness-audit=\"$HOME/Downloads/${stills.projectId}-production-readiness-audit.json\" --output-dir=\"$HOME/Downloads/${stills.projectId}-real-media-visual-qa\"`;
const queue={schemaVersion:"wedding-movie-real-media-visual-correction-queue/v1",authority:"DERIVED_ONLY_FROM_EXPLICIT_HUMAN_REAL_MEDIA_VISUAL_FAIL_VERDICTS",generatedAt:new Date().toISOString(),projectId:stills.projectId,source:{stillsManifestPath:stillsPath,stillsManifestSha256:identity.stillsManifestSha256,humanReviewPath:reviewPath,humanReviewSha256:sha256(reviewPath),identityFingerprint},failures,summary:{failedChecks:failures.length,notRunChecks:notRun,finalVerdict,reviewComplete:notRun===0&&finalVerdict!=="NOT_RUN",correctionRequired:failures.length>0,framingIdentityVerified:true,productionReady:false},nextActions:failures.length?{fixFailedScenes:true,refreshFreshVisualQa:refreshCommand,rerenderRealPreview:`node --no-warnings motion-studio/scripts/render-wedding-project-real-media-preview.mts --selected-manifest=\"$HOME/Downloads/${stills.projectId}-selected-scene-render-manifest.json\" --readiness-audit=\"$HOME/Downloads/${stills.projectId}-production-readiness-audit.json\" --output=\"$HOME/Downloads/${stills.projectId}-real-media-preview.mp4\" --manifest=\"$HOME/Downloads/${stills.projectId}-real-media-preview-manifest.json\" --render`,extractFreshQaStills:`node --no-warnings motion-studio/scripts/wedding-project-real-media-preview-qa-stills.mts --preview-manifest=\"$HOME/Downloads/${stills.projectId}-real-media-preview-manifest.json\"`,initFreshVisualReview:`node --no-warnings motion-studio/scripts/wedding-project-real-media-preview-visual-review.mts --stills-manifest=\"$HOME/Downloads/${stills.projectId}-real-media-preview-qa-stills-manifest.json\" --init`}:null,evidenceBoundary:{humanVisualReviewPerformed:review.evidenceBoundary?.humanVisualReviewPerformed===true,remotionStudioGuiActual:"NOT_RUN",palmierGuiActual:"NOT_RUN",macDaVinciGuiActual:"NOT_RUN",productionReady:false,rule:"Only explicit Human FAIL verdicts become correction work. Framing revision is part of review identity. Mechanical still extraction, CLI render, and CI fixtures never create Human visual PASS or GUI Actual evidence."}};
mkdirSync(dirname(output),{recursive:true});writeFileSync(output,`${JSON.stringify(queue,null,2)}\n`);console.log(JSON.stringify({mode:"finalize",output,projectId:stills.projectId,failures:failures.length,notRun,finalVerdict,framingIdentityVerified:true},null,2));
