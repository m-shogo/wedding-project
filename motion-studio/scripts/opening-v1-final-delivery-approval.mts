...
function verifyApproval(strict: boolean) {
  ...
  try { current = loadCurrentUpstream(); }
  catch (error) { fail(error instanceof Error ? error.message : String(error)); }

  let approval: FinalDeliveryApproval | null = null;
  try { approval = JSON.parse(readFileSync(approvalPath, 'utf8')) as FinalDeliveryApproval; }
  catch { fail('FINAL_DELIVERY_APPROVAL_INVALID_JSON'); }
  if (!approval) {
