...
if (!canonicalIdSet.has(slotId)) fail(`PROFILE_FRAMING_SLOT_UNKNOWN:${slotId}`);
...
if (bothPass && fit !== 'cover' && fit !== 'contain') fail('PROFILE_FRAMING_FIT_REQUIRED_FOR_PASS: use --fit=cover|contain');
...
if (!existsSync(evidencePath)) fail('PROFILE_REAL_MEDIA_REVIEW_EVIDENCE_MISSING: initialize the canonical Human review first');
let evidence: any;
try {
...
if (!Array.isArray(evidence.media) || evidence.media.length !== canonicalIds.length) fail('PROFILE_REAL_MEDIA_REVIEW_MEDIA_CARDINALITY_INVALID');
for (const canonicalId of canonicalIds) {
  if (evidence.media.filter((item: any) => item?.slot === canonicalId).length !== 1) {
    fail(`PROFILE_REAL_MEDIA_REVIEW_SLOT_CARDINALITY:${canonicalId}`);
  }
}
if (evidence.media.some((item: any) => !canonicalIdSet.has(item?.slot))) fail('PROFILE_REAL_MEDIA_REVIEW_UNKNOWN_SLOT');

const media = evidence.media.find((item: any) => item.slot === slotId);
if (!media?.qa || typeof media.qa !== 'object') fail(`PROFILE_REAL_MEDIA_REVIEW_QA_INVALID:${slotId}`);
media.qa.crop = crop;
media.qa.focus = focus;
if (bothPass) {
  media.framing = {fit, ...(focusX !== null && focusY !== null ? {focusX, focusY} : {})};
} else {
...
const sync = spawnSync(process.execPath, ['--no-warnings', 'scripts/sync-profile-v1-framing-verdicts.mts', '--write'], {
  cwd: studioRoot,
  stdio: 'inherit',
});
