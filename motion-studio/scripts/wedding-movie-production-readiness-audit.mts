...
function validateFileBinding(entry: any, allowedExtensions: Set<string>, prefix: string) {
  ...
  if (!/^[a-f0-9]{64}$/i.test(String(entry?.sha256 ?? "")) || actualSha256 !== entry.sha256) {
    return {current: false, blocker: `${prefix}_SHA_MISMATCH`, path, actualSha256};
  }
...
const mediaManifest = mediaManifestPath
  ? loadJson(mediaManifestPath, "MOVIE_PRODUCTION_MEDIA_MANIFEST_NOT_FOUND", "MOVIE_PRODUCTION_MEDIA_MANIFEST_JSON_INVALID")
  : null;
if (mediaManifest && (mediaManifest.schemaVersion !== "wedding-movie-production-media-input/v1" || mediaManifest.projectId !== projectId || !Array.isArray(mediaManifest.scenes))) {
  fail("MOVIE_PRODUCTION_MEDIA_MANIFEST_SCHEMA_MISMATCH");
}
...
const audioManifest = audioManifestPath
  ? loadJson(audioManifestPath, "MOVIE_PRODUCTION_AUDIO_MANIFEST_NOT_FOUND", "MOVIE_PRODUCTION_AUDIO_MANIFEST_JSON_INVALID")
  : null;
if (audioManifest && (audioManifest.schemaVersion !== "wedding-movie-production-audio-input/v1" || audioManifest.projectId !== projectId || audioManifest.usage !== "BGM")) {
  fail("MOVIE_PRODUCTION_AUDIO_MANIFEST_SCHEMA_MISMATCH");
}
...
const scenes = batch.scenes.map((scene: any, index: number) => {
  ...
  if (Number(scene.package?.canonical?.humanState?.canonicalSceneState?.sceneDurationSeconds ?? scene.package?.canonical?.humanState?.resolvedEditableIntent?.sceneDurationSeconds ?? 0) <= 0) {
    blockers.push("SCENE_DURATION_INVALID");
  }

  const mediaEntry = mediaByScene.get(scene.sceneId) ?? null;
  let media = {state: "NOT_PROVIDED", path: null as string | null, sha256: null as string | null, kind: null as string | null, blocker: "REAL_MEDIA_REQUIRED" as string | null};
  if (!mediaEntry) {
    ...
    media = {state: "STALE", path: String(mediaEntry.path ?? ""), sha256: String(mediaEntry.sha256 ?? ""), kind: String(mediaEntry.kind ?? ""), blocker: "STALE_REAL_MEDIA_BINDING"};
  } else if (mediaEntry.kind !== "IMAGE" && mediaEntry.kind !== "VIDEO") {
    blockers.push("REAL_MEDIA_KIND_INVALID");
    media = {state: "INVALID", path: String(mediaEntry.path ?? ""), sha256: String(mediaEntry.sha256 ?? ""), kind: String(mediaEntry.kind ?? ""), blocker: "REAL_MEDIA_KIND_INVALID"};
  } else {
    ...
    media = {state: file.current ? "CURRENT" : "INVALID", path: file.path, sha256: file.actualSha256 ?? String(mediaEntry.sha256 ?? ""), kind: mediaEntry.kind, blocker: file.blocker};
  }

  const transitionAfter = Array.isArray(batch.timeline?.transitions)
    ? batch.timeline.transitions.find((transition: any) => transition.fromSceneId === scene.sceneId) ?? null
    : null;
  if (transitionAfter?.status === "STALE_HUMAN_SELECTION") blockers.push("STALE_HUMAN_SELECTED_TRANSITION");

  const readiness: SceneReadiness = blockers.length > 0
  ...
  return {
    ...
    transitionAfter: transitionAfter ? {
      toSceneId: transitionAfter.toSceneId,
      type: transitionAfter.transition,
      durationFrames: transitionAfter.durationFrames ?? 0,
      status: transitionAfter.status,
    } : null,
    readiness,
    blockers,
    humanInputs: [...new Set(humanInputs)],
    nextAction: readiness === "READY_FOR_REAL_MEDIA_PREVIEW"
      ? "INCLUDE_IN_CONTINUOUS_REMOTION_PRODUCTION_PREVIEW"
      : blockers.length > 0
        ? "REFRESH_STALE_OR_INVALID_AUTHORITY"
        : "COLLECT_HUMAN_COPY_OR_REAL_MEDIA",
  };
});

let audio = {state: "NOT_PROVIDED", path: null as string | null, sha256: null as string | null, blocker: "BGM_AUDIO_REQUIRED" as string | null};
const projectHumanInputs: string[] = [];
const projectBlockers: string[] = [];
if (!audioManifest) {
  projectHumanInputs.push("BGM_AUDIO_REQUIRED");
} else {
...
const identity = {
  ...
  sceneBindings: scenes.map((scene: any) => ({sceneId: scene.sceneId, sourceRevision: scene.sourceRevision, patternId: scene.patternId, productionRole: scene.productionRole, mediaSha256: scene.media.sha256, readiness: scene.readiness})),
...
const audit = {
  ...
  source: {
    ...
    mediaManifestPath,
    mediaManifestSha256: identity.mediaManifestSha256,
    audioManifestPath,
    audioManifestSha256: identity.audioManifestSha256,
    identitySha256: stableSha(identity),
  },
  scenes,
  audio,
  summary: {
    totalScenes: scenes.length,
    readyForRealMediaPreview: readyScenes.length,
    waitingHumanInput: waitingScenes.length,
    blockedByStaleAuthority: blockedScenes.length,
    projectHumanInputs: [...new Set(projectHumanInputs)],
    projectBlockers: [...new Set(projectBlockers)],
  ...
  evidenceBoundary: {
    ...
    productionReady: false,
    rule: "This audit verifies current Human-selected scene authority plus explicitly Human-approved real-media/BGM file bindings. It does not claim visual quality, Remotion Studio GUI Actual, Palmier GUI Actual, Mac DaVinci Actual, or final production approval.",
  },
};
...
if (process.argv.includes("--strict-real-preview-ready") && !readyForContinuousRealMediaPreview) {
  fail("MOVIE_PRODUCTION_REAL_MEDIA_PREVIEW_NOT_READY", `${readyScenes.length}/${scenes.length} scenes; audio=${audio.state}; blockers=${projectBlockers.join(",") || "none"}`);
}
