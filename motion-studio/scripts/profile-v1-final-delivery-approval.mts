...
type Recovery={schemaVersion:'wedding-davinci-production-recovery-export/v1';authority:'FINAL_RENDER_BOUND_DAVINCI_RECOVERY';sourceBundle:{finalRenderSha256:string;realMediaHumanQaEvidenceSha256:string;realMediaHumanQaBindingFingerprintSha256:string};recovery:{movieId:string;stage:string;artifactPath:string;productionReady:false;actual:{state:string;evidencePath:string}}};
...
type Davinci={schemaVersion:'profile-v1-davinci-finishing-evidence/v1';authority:'MAC_DAVINCI_ACTUAL_EVIDENCE';productionRecovery:RecoveryBinding;sourceRender:{path:string;expectedSha256:string};export:{path:string|null;sha256:string|null;watchedWithSound:string};review:{overall:string;reviewer:string|null;reviewedAt:string|null};productionReady:false};
type Approval={schemaVersion:'profile-v1-final-delivery-approval/v1';authority:'HUMAN_FINAL_DELIVERY_APPROVAL';boundAt:string;productionBundle:{path:string;sha256:string};humanFinalRenderReview:{path:string;sha256:string};productionRecovery:RecoveryBinding;davinciEvidence:{path:string;sha256:string};sourceRender:{path:string;sha256:string};davinciExport:{path:string;sha256:string};decision:Decision;approver:string|null;decidedAt:string|null;notes:string;productionReady:boolean};
...
function current(){
  ...
  if(!existsSync(recoveryPath))throw new Error('PROFILE_FINAL_DELIVERY_RECOVERY_SIDECAR_MISSING');
  ...
  if(recovery.schemaVersion!=='wedding-davinci-production-recovery-export/v1'||recovery.authority!=='FINAL_RENDER_BOUND_DAVINCI_RECOVERY')throw new Error('PROFILE_FINAL_DELIVERY_RECOVERY_CONTRACT');
  if(recovery.recovery?.movieId!=='profile'||recovery.recovery?.stage!=='davinciFinishing'||recovery.recovery?.artifactPath!==bundle.finalRender.path)throw new Error('PROFILE_FINAL_DELIVERY_RECOVERY_TARGET_STALE');
  if(recovery.recovery?.productionReady!==false||recovery.recovery?.actual?.state!=='NOT_RUN'||recovery.recovery?.actual?.evidencePath!==rel(davinciPath))throw new Error('PROFILE_FINAL_DELIVERY_RECOVERY_BOUNDARY_INVALID');
  if(recovery.sourceBundle?.finalRenderSha256!==bundle.finalRender.sha256)throw new Error('PROFILE_FINAL_DELIVERY_RECOVERY_RENDER_STALE');
  ...
  if(davinci.productionRecovery?.path!==rel(recoveryPath)||davinci.productionRecovery?.sha256!==recoverySha)throw new Error('PROFILE_FINAL_DELIVERY_DAVINCI_RECOVERY_SHA_STALE');
  if(davinci.productionRecovery?.sourceRenderSha256!==recovery.sourceBundle.finalRenderSha256)throw new Error('PROFILE_FINAL_DELIVERY_DAVINCI_RECOVERY_RENDER_STALE');
  if(davinci.productionRecovery?.realMediaHumanQaEvidenceSha256!==recovery.sourceBundle.realMediaHumanQaEvidenceSha256)throw new Error('PROFILE_FINAL_DELIVERY_DAVINCI_RECOVERY_HUMAN_QA_SHA_STALE');
  if(davinci.productionRecovery?.realMediaHumanQaBindingFingerprintSha256!==recovery.sourceBundle.realMediaHumanQaBindingFingerprintSha256)throw new Error('PROFILE_FINAL_DELIVERY_DAVINCI_RECOVERY_HUMAN_QA_FINGERPRINT_STALE');
  ...
  return{bundle,recovery,finalReview,davinci,bundleSha:sha(bundlePath),recoverySha,finalRenderReviewSha:sha(finalRenderReviewPath),davinciSha:sha(davinciPath)};
}
function init(){
  const c=current(); const a:Approval={schemaVersion:'profile-v1-final-delivery-approval/v1',authority:'HUMAN_FINAL_DELIVERY_APPROVAL',boundAt:new Date().toISOString(),productionBundle:{path:rel(bundlePath),sha256:c.bundleSha},humanFinalRenderReview:{path:rel(finalRenderReviewPath),sha256:c.finalRenderReviewSha},productionRecovery:{path:rel(recoveryPath),sha256:c.recoverySha,sourceRenderSha256:c.recovery.sourceBundle.finalRenderSha256,realMediaHumanQaEvidenceSha256:c.recovery.sourceBundle.realMediaHumanQaEvidenceSha256,realMediaHumanQaBindingFingerprintSha256:c.recovery.sourceBundle.realMediaHumanQaBindingFingerprintSha256},davinciEvidence:{path:rel(davinciPath),sha256:c.davinciSha},sourceRender:{path:c.bundle.finalRender.path,sha256:c.bundle.finalRender.sha256},davinciExport:{path:c.davinci.export.path!,sha256:c.davinci.export.sha256!},decision:'HOLD',approver:null,decidedAt:null,notes:'',productionReady:false};
  mkdirSync(dirname(approvalPath),{recursive:true}); writeFileSync(approvalPath,`${JSON.stringify(a,null,2)}\n`); console.log(`Profile final delivery approval initialized: ${rel(approvalPath)}`); console.log('decision=HOLD productionReady=false — Human final-MP4 review, current recovery chain and Mac DaVinci Actual evidence are SHA-bound; approval is still Human-only.');
}
function verify(strict:boolean){
  ...
  if(a.productionRecovery?.path!==rel(recoveryPath))errors.push('PROFILE_FINAL_APPROVAL_RECOVERY_PATH');
  if(a.davinciEvidence.path!==rel(davinciPath))errors.push('PROFILE_FINAL_APPROVAL_DAVINCI_EVIDENCE_PATH');
  if(c){
    if(a.productionBundle.sha256!==c.bundleSha)errors.push('STALE_PROFILE_FINAL_BUNDLE');
    if(a.humanFinalRenderReview.sha256!==c.finalRenderReviewSha)errors.push('STALE_PROFILE_FINAL_RENDER_REVIEW');
    if(a.productionRecovery?.sha256!==c.recoverySha)errors.push('STALE_PROFILE_FINAL_RECOVERY_SIDECAR');
    if(a.productionRecovery?.sourceRenderSha256!==c.recovery.sourceBundle.finalRenderSha256)errors.push('STALE_PROFILE_FINAL_RECOVERY_RENDER_SHA');
    if(a.productionRecovery?.realMediaHumanQaEvidenceSha256!==c.recovery.sourceBundle.realMediaHumanQaEvidenceSha256)errors.push('STALE_PROFILE_FINAL_RECOVERY_REAL_MEDIA_HUMAN_QA_SHA');
    if(a.productionRecovery?.realMediaHumanQaBindingFingerprintSha256!==c.recovery.sourceBundle.realMediaHumanQaBindingFingerprintSha256)errors.push('STALE_PROFILE_FINAL_RECOVERY_REAL_MEDIA_HUMAN_QA_FINGERPRINT');
    if(a.davinciEvidence.sha256!==c.davinciSha)errors.push('STALE_PROFILE_FINAL_DAVINCI_EVIDENCE');
    if(a.sourceRender.path!==c.bundle.finalRender.path||a.sourceRender.sha256!==c.bundle.finalRender.sha256)errors.push('STALE_PROFILE_FINAL_SOURCE');
    if(a.davinciExport.path!==c.davinci.export.path||a.davinciExport.sha256!==c.davinci.export.sha256)errors.push('STALE_PROFILE_FINAL_EXPORT');
  }
  ...
  if(errors.length){console.log(`Profile final delivery approval: HOLD/BLOCKED (${errors.length})`);for(const e of errors)console.log(`BLOCK / ${e}`);if(strict)process.exit(1);return;}console.log('Profile final delivery approval: APPROVED — Human final-MP4 review, current DaVinci recovery chain and verified Mac DaVinci export are explicitly bound to this Human approval.');
}
