...
if (checker.status !== 0) {
  block('PALMIER_TIMELINE_EXPORT_RECEIPT_NOT_CURRENT', [checker.stdout?.trim(), checker.stderr?.trim()].filter(Boolean).join(' / '));
}
...
if (
  receipt.schemaVersion !== 'wedding-palmier-typography-timeline-export-receipt/v1' ||
  receipt.authority !== 'VERIFIED_PALMIER_TIMELINE_EXPORT_AGAINST_ASSEMBLY_PLAN' ||
  receipt.movieId !== movieId ||
  receipt.state !== 'CURRENT'
) block('PALMIER_TIMELINE_EXPORT_RECEIPT_CONTRACT_INVALID');
...
recovery.palmierTimelineExport = {
  ...
  assemblyPlan: {
    path: displayPath(assemblyPlanPath),
    sha256: assemblyPlanSha256,
  },
  palmierFcpxml: {
    path: displayPath(fcpxmlPath),
    sha256: fcpxmlSha256,
  },
  verification: {
