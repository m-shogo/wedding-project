...
素材登録欄の `IMAGE ADVISOR` は、Hero・旅・家族友人・会場・小物ごとに次を表示する。
...
登録後の素材カードでは「この画像へのアドバイス」を開くと、横構図・文字余白・Hero指定など現在の設定に合わせた強みと確認事項が表示される。これは候補を考える補助であり、AIが写真を自動採用するものではない。

### Roughをレビュー画面へ自動接続

制作ワークスペースの「Renderを直す」で「Rough作成＋接続コマンドをコピー」を使う。

```sh
cd movie-dashboard
pnpm prepare:start-review
pnpm dev
```

`prepare:start-review` は `StartExtendedOpeningRoughV1` を低解像度renderし、最新ファイルをGit管理外のローカルレビューリンクへ接続する。原本MP4は移動・上書きしない。画面を再読み込みすると最新Roughが自動接続され、再生位置付きコメントを残せる。
