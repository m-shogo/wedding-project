...
### 追記3: 連続モーション化 + 「全部進めて」の切り分け（2026-08-09）

要望「全部進めて（テンポ / クロスディゾルブ / BGM / 実写真）」に対し、実行可否で切り分けた。

- **実行済み: 地域内カットの連続モーション化**
  同一ダミー画像を3カットに割ると各カットでズーム量がジャンプして不自然だったため、
  地域ごと（Okinawa / Seoul / Hawaii）に3クリップを**1本の連続Ken Burns**として再設定。
  地域内のカットは画が途切れず滑らか、地域の変わり目は別画像のクリーンカット。
  Okinawaズームイン / Seoulズームアウト / Hawaiiズームイン / Hero緩やかpush、で緩急を付与。
  再書き出し: `~/Downloads/Opening V1 3.mp4`（H.264 / 1080p / 60.0s）。
- **保留: テンポアップ（3秒→2秒等）**
  ダミー画像が地域あたり1枚しかなく、速いカット＝同じ画の連打になり不自然。
  真のテンポアップは**実写真（地域あたり複数枚）**が入ってから行うのが適切。
  正本の11枠・地域境界（9/18/27/36/44/52s）は崩していない。
- **保留: クロスディゾルブ**
  referenceが写真区間の下で自前のplaceholderカードを表示するため、単純な不透明度フェードは
  下のreferenceが透けて破綻する。真のクロスディゾルブには写真を2トラックに分けた再構成が必要。
  実写真投入時にまとめて実施するのが安全（今のダミー段階で構成を作り込みすぎない）。
- **ブロック: BGM**
  BGM音源ファイルが未提供。`canGenerate=false`（Palmier未サインイン）かつ有料生成は不可・
  権利未確認のため、AI生成もしない。**BGMファイルの提供待ち**。
- **ブロック: 実写真11枚**
  0/11のまま。実写真ファイルの提供待ち。届き次第、上記clipIdへ差し替え、
  そのタイミングでテンポ調整とクロスディゾルブも本適用する。
