...
素材全体はよくても、隅にロゴ・機体・車両が写ることがある。
その場合は捨てずに、`crop` 列で該当部分を外す。
...
- ffmpegと同じ `W:H:X:Y`。切り出し後に1920x1080へ戻す。
- 16:9を保つ値にすると歪まない（例: 高さ990なら幅1760）。
- `validate` が形式と1920x1080からのはみ出しを検証する。
- 実例: `clip-004` は右上に機体の翼が全編写っていたので上部を外した。

**crop指定があると `--copy` でも再エンコードされる**（cropは再エンコードが必須のため）。
