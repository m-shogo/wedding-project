...
need(model, 'return defaultWeddingDavinciGuiActualStartGateAudits[projectId].inspectCommand;', "critical path command must be exactly the Start Gate Audit inspect command");
...
need(criticalPathCard, '{target.command}', "critical-path card must display the already-canonical executable command verbatim");
...
console.log("DaVinci critical-path Start Gate contract OK: Critical Path consumes the Start Gate Audit command/path authority, renders the canonical saved artifact target, and preserves Session Plan, Project Motion revalidation, strict GUI-start verification and NOT_RUN evidence boundaries.");
