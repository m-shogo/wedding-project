...
requireText(generationPacket, "return buildVideoProviderFields(prompt).mainPrompt;", "provider model input must stay limited to the clean main prompt");
requireText(generationPacket, "buildVideoOperatorPacket", "operator metadata must remain separate from the provider main prompt");
requireText(generationPacket, "QA ONLY UNTIL PROVIDER POLICY IS VERIFIED", "unknown negative policy must fail safe into human QA only");
requireText(generationPacket, "[OPTIONAL SEPARATE NEGATIVE FIELD — USE ONLY IN A SEPARATE PROVIDER FIELD]", "optional negative text must stay visibly separated from the main prompt");
...
requireText(promptBuilder, 'modelId === "runway-gen-4.5" && intent.mode === "i2v"', "motion-first compilation must be scoped to Runway I2V rather than all modes");
requireText(promptBuilder, "runway-i2v-input=motion-first", "Runway I2V prompts must retain an auditable compiler marker");
requireText(promptBuilder, "function cameraInstruction", "camera controls must compile through a sentence-normalization layer");
requireText(promptBuilder, '"slow push-in": "Camera performs one slow, steady push-in."', "common camera preset shorthand must compile into a complete natural-language sentence");
requireText(promptBuilder, "cameraInstruction(intent.camera)", "compiled motion must use normalized camera sentences");
forbidText(promptBuilder, "intent.camera.trim(),", "raw camera shorthand must not be inserted directly into compiled model prompts");
requireText(promptBuilder, "function sentenceInstruction", "free-form motion instructions must retain explicit sentence boundaries");
requireText(promptBuilder, "sentenceInstruction(intent.action)", "action text must be normalized before it is joined with camera or pacing instructions");
requireText(promptBuilder, "function prefixedInstruction", "reference guidance must avoid missing or duplicated sentence terminators");
forbidText(promptBuilder, "intent.action.trim(),", "raw action shorthand must not be inserted directly into compiled model prompts");
requireText(promptBuilder, "function inlineFragment", "embedded lighting fragments must remove trailing sentence punctuation before wrapping");
requireText(promptBuilder, 'prefixedInstruction("Mood", intent.mood)', "mood text must use the shared sentence boundary helper");
requireText(promptBuilder, "function instructionClauseCount", "prompt complexity warnings must share one clause counter");
requireText(promptBuilder, "split(/[,;、；。]/)", "prompt complexity warnings must count comma and semicolon separated clauses");
requireText(promptBuilder, "runwayI2VIgnoresAppearanceControls", "warnings must recognize that Runway I2V omits appearance controls from its main prompt");
...
requireText(failureTaxonomy, "(^|[^a-z0-9])", "ASCII failure keywords must retain token boundaries to avoid substring false positives");
...
requireText(preflight, "runway-i2v-motion-first-missing", "legacy Runway I2V drafts without the motion-first marker must stay visible before generation");
requireText(preflight, 'lastNoteValue(prompt.notes, "runway-i2v-input")', "Runway migration preflight must read the latest compiler marker");
requireText(preflight, "既に生成を開始しているなら追加生成せず", "testing-state migration guidance must avoid duplicate provider generation");
...
requireText(assetReprobe, "採用正本はfingerprintなしのprobe証跡で上書きできません", "adopted result evidence must not be overwritten by an incomplete fingerprint probe");
requireText(assetReprobe, "QA時fingerprintと変わったため結果レビューで再QAが必要です", "changed adopted media fingerprint must explicitly require visual re-review");
requireText(assetReprobe, "disabled={!pendingEvidence || wouldRemoveAdoptedFingerprint}", "reprobe save control must block adopted-authority fingerprint removal");
requireText(assetReprobe, "fingerprint一致だけで目視QA PASSを推測せず", "fingerprint equality must never be presented as automatic visual QA");
...
requireText(queue, "IMPORTANT: DO NOT PASTE THIS WHOLE PACKET INTO ONE PROVIDER PROMPT.", "batch handoff must explicitly forbid pasting multiple shots into one provider prompt");
requireText(queue, "Run each SHOT separately", "batch handoff must require one provider generation per shot");
requireText(queue, "[MAIN PROMPT — RUN THIS SHOT SEPARATELY]", "each batch shot must mark the exact main prompt boundary");
requireText(queue, "[OPTIONAL SEPARATE NEGATIVE FIELD — USE ONLY IN THE PROVIDER NEGATIVE FIELD]", "batch handoff must preserve optional negative-field separation");
requireText(queue, "buildVideoProviderFields(prompt)", "batch and JSON handoff must use structured provider fields");
forbidText(queue, 'runnable.map(modelInput).join("\\n\\n---\\n\\n")', "raw main prompts must never be concatenated into one ambiguous batch provider payload");
requireText(queue, "batchは各SHOTを別生成。丸ごとproviderへ貼らない", "queue UI must keep the separate-shot batch warning visible before copy");
