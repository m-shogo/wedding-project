...
for (const token of [
  ...
  'FILL_FROM_MAC_ACTUAL_SESSION',
  'IMPLEMENTATION_ID_MISMATCH',
  'DUPLICATE_PATTERN_ID',
  'UNKNOWN_PATTERN_ID',
  'NINE_PATTERN_COVERAGE_INCOMPLETE',
...
]) requireText(session, token, `Actual session contract missing: ${token}`);

for (const token of [
  "buildTypographyDaVinciActualSessionTemplateJson",
  "typography-davinci-actual-session-template.json",
  "NOT_RUN session templateを保存",
  "evidence欄はすべてNOT_RUN",
...
requireText(plan, 'authority: "PLAN_ONLY_NOT_ACTUAL_EVIDENCE"', "Plan and session evidence authorities must remain distinct");
...
if (/automaticPromotionAllowed:\s*true/.test(session) || /productionReady:\s*true/.test(session)) errors.push("Session evaluator must never auto-promote to production");

const passGuardTokens = [
...
for (const token of passGuardTokens) requireText(session, token, `Human-review eligibility missing evidence guard: ${token}`);
...
console.log("Typography DaVinci Actual session contracts OK: the 9-pattern NOT_RUN template round-trips Mac evidence into a non-mutating Dashboard validator, requires raw evidence + canonical parity + all bindings + 1x/half-speed QA + reviewedAt for PASS, and never auto-promotes production state.");
