...
if(process.argv.includes("--write")){fs.writeFileSync(outputPath,output,"utf8");console.log(`Profile production status synced: ${snapshot.overallState}, palmier=${snapshot.handoff.palmier.contractVersion}, productionReady=${snapshot.readiness.productionReady}`);process.exit(0)}
...
console.log(`Profile production status current: ${snapshot.overallState}, palmier=${snapshot.handoff.palmier.contractVersion}, productionReady=${snapshot.readiness.productionReady}`);
