...
const statusPath=path.join(repoRoot,"motion-studio/scripts/profile-v1-production-status.mts");
...
const snapshot={
  source:{status:"motion-studio/scripts/profile-v1-production-status.mts"},
...
if(process.argv.includes("--write")){fs.writeFileSync(outputPath,output,"utf8");console.log(`Profile production status synced: ${snapshot.overallState}`);process.exit(0)}
...
console.log(`Profile production status current: ${snapshot.overallState}, productionReady=${snapshot.readiness.productionReady}`);
