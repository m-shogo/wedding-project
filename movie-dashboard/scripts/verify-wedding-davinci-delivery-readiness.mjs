...
if (!preflightSource.includes('snapshot.current && live.strictDeliveryEligible && finalEvidenceManifest.current && blockerCodes.length === 0')) fail('Dashboard final delivery eligibility must require current snapshot + live Wedding eligibility + CURRENT final evidence manifest + zero blockers');
...
console.log(`Wedding DaVinci readiness + transition Actual + completion receipt + Project Motion + final evidence manifest + operator packet surface OK: state=${report.state} opening=${report.opening.nextGate}/${report.opening.transitionGate.state}/${report.opening.completionGate.state} profile=${report.profile.nextGate}/${report.profile.transitionGate.state}/${report.profile.completionGate.state}`);
