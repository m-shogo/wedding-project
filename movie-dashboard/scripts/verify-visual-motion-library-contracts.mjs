...
requireAll(page, [
  'Native App Actual 7件を、検証根拠と一緒に公開する',
  'Mask Reveal・Quiet Caption・Static Hero・Word Punch・Small Push・Flash SoftはDaVinci Actual、Hard Cut AccentはPalmier Actualまで到達済み',
...
console.log(`Visual Motion Library contracts OK (${actualMediaState.mode === "COMPLETE" ? "media verified" : "code-only; local media absent"}): 7 native-app Actuals (mask-reveal, quiet-caption, cut-hard-accent, photo-static-hero, type-word-punch, photo-small-push, flash-one-frame-soft), 2 source-media Actual TESTED renders, independent pixel-oracle evidence, and separate Concept assets are registered; Human Master Scene values remain the source of truth.`);
