...
for(const t of ['opening-v1-production-status.mts','opening-v1-production-status/v1','DERIVED_PRODUCTION_STATUS','openingProductionStatus.generated.ts','productionReady','nextActions'])need(sync,t,`Opening status sync missing ${t}`);
for(const t of ['"overallState": "MEDIA_REQUIRED"','"media": {','"previewRender": {','"previewReview": {','"finalRender": {','"productionBundle": {','"davinciFinishing": {','"finalDeliveryApproval": {','"state": "BLOCKED"','"state": "NOT_RUN"','"macDaVinciActualVerified": false','"finalDeliveryApproved": false','"productionReady": false','"nextActions": [','"pnpm sync:photos"','"pnpm opening:assembly-preflight"'])need(generated,t,`generated Opening status missing ${t}`);
for(const t of ['wedding-opening-production-status-handoff/v1','MOTION_STUDIO_DERIVED_OPENING_STATUS_HANDOFF','openingProductionGate.expectedPhotoCount','openingProductionGate.photoSlots.map','preferredFileStem: slot.key','intakeDirectory: "motion-studio/public/photos/opening/"','openingProductionGate.bgm','openingProductionStatus.overallState','openingProductionStatus.stages','openingProductionStatus.readiness','openingProductionStatus.nextActions','nextActions: [...openingProductionStatus.nextActions]','STATUS_EXPORTABLE != FINAL_RENDER_ELIGIBLE','DAVINCI_ACTUAL_VERIFIED != FINAL_DELIVERY_APPROVED','MEDIA_REQUIREMENT_EXPORTED != MEDIA_RESOLVED','NEXT_ACTION_EXPORTED != ACTION_COMPLETED'])need(handoff,t,`Opening handoff envelope missing ${t}`);
for(const t of ['buildOpeningProductionStatusHandoff()','buildOpeningProductionStatusHandoffJson()','projectId !== "opening"','Opening production statusを書き出す','opening-production-status-handoff.json','production.overallState','production.readiness.productionReady','production.readiness.macDaVinciActualVerified','Object.entries(production.stages)','NEXT ACTIONS','production.nextActions.map','MEDIA_REQUIRED / NOT_RUNも含めて現在状態を外へ渡す','NEXT_ACTION_EXPORTED != ACTION_COMPLETED'])need(card,t,`Opening status UI missing ${t}`);
...
console.log('Opening production-status handoff OK: Motion Studio state, 11-slot intake requirements, and executable next actions reach the Motion Zukan UI/export while unresolved media and Mac/production readiness remain fail-closed.');
