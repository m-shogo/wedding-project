...
if(process.argv.includes("--write")){fs.writeFileSync(outputPath,output,"utf8");console.log(`Opening production status synced: ${snapshot.overallState}, davinci=${snapshot.handoff.davinci.contractVersion}`);process.exit(0)}
...
console.log(`Opening production status current: ${snapshot.overallState}, palmier=${snapshot.handoff.palmier.contractVersion}, davinci=${snapshot.handoff.davinci.contractVersion}, productionReady=${snapshot.readiness.productionReady}`);
