...
if(process.argv.includes("--write")){fs.writeFileSync(outputPath,output,"utf8");console.log(`Opening production status synced: ${snapshot.overallState}, previewSource=${snapshot.sourceRevalidation.realMediaPreview.state}, davinci=${snapshot.handoff.davinci.contractVersion}`);process.exit(0)}
...
console.log(`Opening production status current: ${snapshot.overallState}, previewSource=${snapshot.sourceRevalidation.realMediaPreview.state}, palmier=${snapshot.handoff.palmier.contractVersion}, davinci=${snapshot.handoff.davinci.contractVersion}, productionReady=${snapshot.readiness.productionReady}`);
