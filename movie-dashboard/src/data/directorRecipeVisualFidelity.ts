...
export const motionPresetVisualSupport: MotionPresetVisualSupport[] = [
  ...
  support("wipe-route-line", "representative", "2026-08-26: TransitionWipeEngine's dedicated `route-line` variant draws the line and the leading dot from the same cubic-bezier control points and the same progress value (routeLineMath.ts), so the dot tracks the curve's real tip instead of a separate percent-based approximation; verified for all 4 directions (left/right/up/down) by local Remotion render, including confirming the curve stays within the viewBox and that left/up correctly reverse traversal instead of only mirroring the dot. Both StartMotionReel.tsx and the Director Recipe preview now share the same resolveTransitionWipeProps() resolution, and a regression check (check-start-motion-kit.mts) exercises the real geometry/resolver functions instead of grepping source text. Kept at representative (not exact) because this preset is a required non-exact sentinel in check-director-recipe-visual-fidelity.mts pending the same independent rendered-pixel oracle used for the Phase J3 exact-promotion batch — manual eyeball confirmation and a regression check alone do not satisfy that gate.", "Add an independent rendered-pixel oracle assertion (distinct line/dot pixels, dot on the curve) before promoting to exact, following the photo-2p5d-parallax / accent-halftone-burst precedent."),
