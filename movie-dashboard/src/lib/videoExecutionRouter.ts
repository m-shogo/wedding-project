...
export function routeVideoPrompt(prompt: Prompt, resultAssets: Asset[]): VideoExecutionRoute {
  ...
  if (prompt.status === "testing" && resultAssets.length === 0) {
    return {
      ...
      label: "生成結果待ち",
      reason: "PromptはtestingだがresultAssetが未登録。既存結果の登録または生成準備を整理する段階。",
  ...
  return {
    destination: "external-generation",
    label: "低コスト試作候補",
    reason: "draftのため、shot intentを確認するための低コスト試作候補を準備する段階。生成実行の許可とは分離する。",
...
export function buildPalmierAgentHandoff(params: {
  ...
  const markdown = [
    ...
    "- Browser-measured duration/resolution are local media evidence, not visual QA; preserve them separately from manually entered result metadata.",
    ...
    ...rows.flatMap((row) => [
      ...
      row.resultAssets.some((asset) => asset.probe?.measuredDurationSec || asset.probe?.measuredResolution) ? `- browser-measured media: ${row.resultAssets.map((asset) => `${asset.probe?.measuredDurationSec ?? "?"}s / ${asset.probe?.measuredResolution || "?"} / probed=${asset.probe?.probedAt || "—"}`).join(" / ")}` : "",
