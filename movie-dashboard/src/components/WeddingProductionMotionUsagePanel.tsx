  ...
  return (
    <section className="mb-8 border border-sand-300 bg-white p-5 dark:border-navy-600 dark:bg-navy-800" aria-label="Wedding productionで実使用中のMotion">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-navy-600 dark:text-navy-300">
            ...
            project assignmentは適合度から推測せず、ここでHuman Masterが明示的にOpening/Profileへ割り当てた記録だけを使用する。
          </p>
        </div>
            ...
            return (
              <article key={record.patternId} className="border border-sand-200 p-4 dark:border-navy-600">
                ...
                <button
                  ...
                  className={`mt-4 border px-3 py-2 text-xs font-semibold ${assigned ? "border-red-300 text-red-700 dark:border-red-900 dark:text-red-300" : "border-sky-500 text-sky-700 dark:text-sky-300"}`}
                >
      ...
      <p className="mt-4 border-l-2 border-amber-400 pl-3 text-[11px] leading-5 text-navy-500 dark:text-navy-300">
        Authority boundary: usageStageはWedding全体のROUGH/FINAL採用、project assignmentはHuman Master workspaceの明示操作だけを正本にする。fit metadataだけでは割り当てない。割当操作はMotionの実装検証・Human QA・Remotion Studio GUI Actual・Mac DaVinci GUI Actualを昇格させない。
      </p>
    </section>
  );
}