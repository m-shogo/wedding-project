import {buildOpeningProductionStatusHandoff, buildOpeningProductionStatusHandoffJson} from "../data/openingProductionStatusHandoff";
...
export function OpeningProductionHandoffExportButton({compact = false}: {compact?: boolean}) {
  ...
  const nextGate = handoff.opening.production.nextGate;
  ...
  return <div className={compact ? "inline-block" : "w-full border border-violet-300 dark:border-violet-700 px-3 py-3"}>
    ...
    {!compact ? <ProductionNextGateSummary projectLabel="Opening" state={nextGate.state} stage={nextGate.stage} artifactPath={nextGate.artifactPath} blockerCodes={nextGate.blockerCodes} blockerActions={nextGate.blockerActions} recovery={nextGate.recovery} actionTargets={nextGate.actionTargets} /> : null}
  </div>;
}
