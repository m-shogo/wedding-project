...
import {buildOpeningEffectiveProductionHandoffJson} from "../lib/effectiveProductionHandoffExport";
...
export function OpeningProductionHandoffExportButton({compact = false}: {compact?: boolean}) {
  ...
  const exportHandoff = () => downloadText(buildOpeningEffectiveProductionHandoffJson(), "opening-v1-production-handoff.json");
  return <div className={compact ? "inline-block" : "w-full border border-violet-300 dark:border-violet-700 px-3 py-3"}>
    <button type="button" onClick={exportHandoff} className={compact ? "px-3 py-2 border border-violet-300 dark:border-violet-700 text-xs font-semibold text-violet-700 dark:text-violet-300" : "w-full text-left"}>
      ...
      <span className="mt-1 block text-sm font-bold text-navy-900 dark:text-sand-100">{compact ? "Production Handoff JSONを書き出す" : "11写真・BGM/ambience・critical path・effective next gate・Palmier / DaVinci状態を1 JSONへ"}</span>
