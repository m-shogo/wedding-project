    ...
    <section className="mt-3 border-t border-sand-200 dark:border-navy-600 pt-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[9px] tracking-[0.16em] font-semibold text-violet-700 dark:text-violet-300">HUMAN MASTER HANDOFF</p>
          ...
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-[10px] text-navy-500 dark:text-navy-300">
            ...
            <span>Palmier XML: {bundle.timeline.projectTimelineXmlFileName}</span>
            <span>Sidecar: {bundle.timeline.sidecarFileName}</span>
        ...
        <div className="flex flex-wrap gap-2 shrink-0">
          <button type="button" onClick={() => void copyJson()} className="border border-violet-300 dark:border-violet-800 px-2.5 py-1.5 text-[10px] font-semibold text-violet-700 dark:text-violet-300">
            {copied ? "COPIED ✓" : "Handoff JSONをコピー"}
          </button>
      ...
      <p className="mt-2 text-[10px] leading-4 text-navy-400">
        Sceneを編集するとupdatedAtが変わり、この表示bundleも現在のSceneInstanceから再生成されます。NLE XML自体はPalmier実timelineからexportします。
      </p>

      <details className="mt-2">
        <summary className="cursor-pointer text-[10px] text-sky-700 dark:text-sky-300">Handoff詳細を見る</summary>
