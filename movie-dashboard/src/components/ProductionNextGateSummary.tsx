...
function BlockerAction({action}: {action: MovieProductionBlockerRecoveryAction}) {
  ...
  return <span title={action.purpose} className="border border-red-300 px-2 py-1 text-[10px] text-red-700 dark:border-red-800 dark:text-red-300">{action.label}</span>;
}

export function ProductionNextGateSummary({projectLabel, state, stage, artifactPath, blockerCodes, blockerActions, recovery, actionTargets}: ProductionNextGateSummaryProps) {
  ...
  return (
    <div className="mt-3 border-t border-violet-200 pt-3 dark:border-violet-800">
      ...
      {blockerActions.length > 0 ? <div className="mt-2"><p className="mb-1 text-[10px] font-semibold tracking-[0.12em] text-red-500 dark:text-red-300">BLOCKER RECOVERY</p><div className="flex flex-wrap gap-1.5">{blockerActions.map((action) => <BlockerAction key={action.id} action={action} />)}</div></div> : null}
