...
export function WordPunchDaVinciEvidencePanel({
  ...
  function exportTemplate() {
    downloadText(
      ...
      `${scene.sceneId}-type-word-punch-davinci-evidence-capture.json`,
    );
  }
  ...
  function exportEvaluated() {
    ...
    downloadText(
      ...
      `${scene.sceneId}-type-word-punch-davinci-evaluated-evidence.json`,
    );
  }
  ...
  return (
    <div className="mt-2 border border-fuchsia-200 dark:border-fuchsia-800 p-2.5 text-[9px] leading-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="font-semibold text-fuchsia-700 dark:text-fuchsia-300">Word Punch / whole-title Transform Actual</p>
          <p className="mt-1 text-navy-400">
            Text+ + whole-title Transform expected / live parameter binding: NOT_VERIFIED / GUI・Readback・Render・Visual QA: NOT_RUN
          </p>
        </div>
      ...
      <p className="mt-2 text-navy-400">
        canonical punchは文字単位Followerではなくタイトル全体のscale + opacityです。Mac Resolve ActualではText+ / Transformのlive input名・start/end frame・scale・opacity・easingを実測し、推測値では埋めません。
      </p>
