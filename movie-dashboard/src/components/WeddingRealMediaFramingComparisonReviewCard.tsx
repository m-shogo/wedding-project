...
type ComparisonCurrentness = {
  schemaVersion: "wedding-movie-real-media-framing-qa-comparison-currentness/v1";
  authority: "LIVE_REVALIDATION_OF_FRAMING_COMPARISON_AGAINST_SOURCE_MANIFESTS_STILLS_AND_CURRENT_MEDIA";
  projectId: SceneProjectId;
  state: "CURRENT" | "STALE";
  source: {receiptSha256: string | null; currentMediaSha256: string | null};
  checks: {receiptBoundaryValid: boolean; sourceManifestHashesCurrent: boolean; stillFilesCurrent: boolean; currentMediaBindingCurrent: boolean; receiptManifestBindingCurrent: boolean};
  mismatches: string[];
  evidenceBoundary: {humanVisualReviewPerformed: false; humanVisualQa: "NOT_RUN"; remotionStudioGuiActual: "NOT_RUN"; palmierGuiActual: "NOT_RUN"; macDaVinciGuiActual: "NOT_RUN"; productionReady: false};
};
...
function isCurrentness(value: unknown, projectId: SceneProjectId): value is ComparisonCurrentness {
  if (!value || typeof value !== "object") return false;
  const currentness = value as Partial<ComparisonCurrentness>;
  return currentness.schemaVersion === "wedding-movie-real-media-framing-qa-comparison-currentness/v1"
    && currentness.authority === "LIVE_REVALIDATION_OF_FRAMING_COMPARISON_AGAINST_SOURCE_MANIFESTS_STILLS_AND_CURRENT_MEDIA"
...
export function WeddingRealMediaFramingComparisonReviewCard({projectId}: {projectId: SceneProjectId}) {
  ...
  const [mediaSha, setMediaSha] = useState<string | null>(null);
  const [strictCurrentness, setStrictCurrentness] = useState<ComparisonCurrentness | null>(null);
  ...
  async function loadMedia(file: File | null) {
    ...
    try {
      ...
      setMedia(value);
      setMediaSha(await createHash(buffer));
      setStrictCurrentness(null);
    } catch (reason) {
      setMedia(null);
      setMediaSha(null);
  ...
  async function loadCurrentness(file: File | null) {
    ...
    try {
      ...
      if (!isCurrentness(value, projectId)) throw new Error("comparison currentness schema / authority / evidence boundary mismatch");
      setStrictCurrentness(value);
    } catch (reason) {
      setStrictCurrentness(null);
      setError(reason instanceof Error ? reason.message : "invalid comparison currentness");
    }
  }
  ...
  const strictBinding = useMemo(() => {
    if (!strictCurrentness) return {state: "NOT_RUN" as const, mismatch: "STRICT_CURRENTNESS_NOT_LOADED"};
    if (!receiptSha || !mediaSha) return {state: "NOT_RUN" as const, mismatch: "RECEIPT_OR_MEDIA_SHA_NOT_LOADED"};
    if (strictCurrentness.source.receiptSha256 !== receiptSha) return {state: "STALE" as const, mismatch: "CURRENTNESS_RECEIPT_SHA_MISMATCH"};
    if (strictCurrentness.source.currentMediaSha256 !== mediaSha) return {state: "STALE" as const, mismatch: "CURRENTNESS_MEDIA_SHA_MISMATCH"};
    if (strictCurrentness.state !== "CURRENT" || strictCurrentness.mismatches.length > 0) return {state: "STALE" as const, mismatch: strictCurrentness.mismatches.join(" / ") || "STRICT_CURRENTNESS_STALE"};
    if (!Object.values(strictCurrentness.checks).every(Boolean)) return {state: "STALE" as const, mismatch: "STRICT_CURRENTNESS_CHECK_FALSE"};
    return {state: "CURRENT" as const, mismatch: null};
  }, [strictCurrentness, receiptSha, mediaSha]);
  ...
  const canReview = browserBinding.state === "CURRENT" && strictBinding.state === "CURRENT" && Boolean(selected?.framingChanged) && Boolean(beforeImage && afterImage);
  const receiptPath = `out/qa/project-real-media-preview/${projectId}/${projectId}-framing-before-after.json`;
  const mediaPath = `out/production/${projectId}/${projectId}-production-media-input.json`;
  const currentnessPath = `out/qa/project-real-media-preview/${projectId}/${projectId}-framing-before-after-currentness.json`;
  const strictCommand = `node --no-warnings motion-studio/scripts/verify-wedding-project-real-media-framing-qa-comparison-currentness.mts --receipt=${receiptPath} --current-media=${mediaPath} --strict-current --output=${currentnessPath}`;

  return (
    <section className="mt-3 border-2 border-violet-300 p-3 dark:border-violet-800" data-real-media-framing-comparison-review={projectId} data-comparison-current={browserBinding.state} data-comparison-strict-current={strictBinding.state} data-human-visual-qa="NOT_RUN">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div><p className="text-[8px] font-semibold tracking-[0.14em] text-violet-700 dark:text-violet-300">FRAMING BEFORE / AFTER VISUAL REVIEW / {projectId.toUpperCase()}</p><p className="mt-1 text-[11px] font-semibold">同一Scene・同一mediaのcrop修正だけを実stillで比較</p><p className="mt-1 text-[8px] opacity-70">comparison receipt + current media + canonical strict-current + SHA一致stillを要求。どちらが良いかのHuman判定は自動生成しません。</p></div>
        <div className="flex gap-1"><span className="border border-violet-300 px-2 py-1 font-mono text-[8px]">LIVE {browserBinding.state}</span><span className="border border-violet-300 px-2 py-1 font-mono text-[8px]">STRICT {strictBinding.state}</span></div>
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-[8px]">
        ...
        <label className="cursor-pointer border border-violet-300 px-2 py-1">Strict currentness<input className="hidden" type="file" accept="application/json,.json" onChange={(event) => void loadCurrentness(event.target.files?.[0] ?? null)} /></label>
      ...
      {browserBinding.state === "WAITING_MEDIA" ? <p className="mt-2 border border-amber-300 p-2 text-[8px] text-amber-800">LIVE currentness未確定: 現在のproduction media manifestを読み込んでください。</p> : null}
      {browserBinding.state === "STALE" ? <p className="mt-2 border border-rose-300 p-2 text-[8px] text-rose-700" data-comparison-stale-scenes={browserBinding.stale.join(",")}>STALE_FRAMING_COMPARISON: {browserBinding.stale.join(" / ")}。Scene revision / media SHA / current framing revisionがdriftしています。</p> : null}
      {strictBinding.state !== "CURRENT" ? <p className="mt-2 border border-amber-300 p-2 text-[8px] text-amber-800" data-comparison-strict-blocker={strictBinding.mismatch ?? "UNKNOWN"}>STRICT_CURRENTNESS_REQUIRED: {strictBinding.mismatch}。canonical verifierがCURRENTで、かつ読み込んだreceipt/mediaのSHAと一致するまでHuman再reviewを解放しません。</p> : null}

      {receipt ? <div className="mt-3 grid gap-2 sm:grid-cols-4 text-[8px]"><div className="border p-2"><p className="font-semibold">Scenes</p><p className="font-mono">{receipt.summary.sceneCount}</p></div><div className="border p-2"><p className="font-semibold">Framing changed</p><p className="font-mono">{receipt.summary.framingChangedScenes}</p></div><div className="border p-2"><p className="font-semibold">Verified stills loaded</p><p className="font-mono">{images.size}/{loadedExpectedCount}</p></div><div className="border p-2"><p className="font-semibold">Strict currentness</p><p className="font-mono">{strictBinding.state}</p></div></div> : null}
      ...
      {selected ? <>
        <div className="mt-3 grid gap-2 lg:grid-cols-2">
          {(["before", "after"] as const).map((side) => <div key={side} className="border p-2" data-framing-comparison-side={side}><div className="flex items-center justify-between gap-2 text-[8px]"><span className="font-semibold uppercase">{side}</span><code className="max-w-[70%] break-all text-[6px]">{selected[side].framingRevision}</code></div><p className="mt-1 text-[8px]">{framingLabel(selected[side].framing)}</p></div>)}
        </div>
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-2" data-framing-comparison-image-pair={selected.sceneId}>
          <figure className="border p-2"><figcaption className="mb-2 text-[8px] font-semibold">BEFORE · {selectedKind}</figcaption>{beforeImage ? <img src={beforeImage.url} alt={`${selected.sceneId} before ${selectedKind}`} className="aspect-video w-full bg-black object-contain" /> : <div className="flex aspect-video items-center justify-center border border-dashed text-[8px] opacity-60">SHA一致before still未読込 · {beforeStill ? basename(beforeStill.path) : "missing receipt sample"}</div>}{beforeStill ? <code className="mt-2 block break-all text-[6px]">frame {beforeStill.frame} · {beforeStill.sha256}</code> : null}</figure>
          <figure className="border p-2"><figcaption className="mb-2 text-[8px] font-semibold">AFTER · {selectedKind}</figcaption>{afterImage ? <img src={afterImage.url} alt={`${selected.sceneId} after ${selectedKind}`} className="aspect-video w-full bg-black object-contain" /> : <div className="flex aspect-video items-center justify-center border border-dashed text-[8px] opacity-60">SHA一致after still未読込 · {afterStill ? basename(afterStill.path) : "missing receipt sample"}</div>}{afterStill ? <code className="mt-2 block break-all text-[6px]">frame {afterStill.frame} · {afterStill.sha256}</code> : null}</figure>
        </div>
        <p className={`mt-3 border-2 p-2 text-[8px] ${canReview ? "border-emerald-400 text-emerald-800 dark:text-emerald-200" : "border-amber-300 text-amber-800 dark:text-amber-200"}`} data-framing-comparison-review-ready={canReview ? "true" : "false"}>{canReview ? "HUMAN REVIEW READY — canonical strict-current + live authority + before/after still SHA VERIFIED。PASSは既存fresh Human Visual Review OperatorでNOT_RUNから入力してください。" : "HUMAN REVIEW BLOCKED — strict-current / live currentness / selected before-after SHA verificationが全部揃うまで再判定しません。"}</p>
      </> : null}

      <div className="mt-3 border border-slate-300 p-2 text-[7px] leading-3"><p className="font-semibold">Canonical strict-current</p><code className="mt-1 block break-all">{strictCommand}</code><p className="mt-1">生成した <code>{currentnessPath}</code> を上のStrict currentnessへ読み込んでください。</p></div>
      <p className="mt-2 border-l-2 border-amber-300 pl-2 text-[7px] leading-3 text-amber-800 dark:text-amber-200">Evidence boundary: comparison/currentness/SHA一致stillはcrop修正前後を安全に並べるderived evidenceです。Human visual QA PASS、Remotion Studio GUI Actual、Palmier GUI Actual、Mac DaVinci GUI Actual、productionReadyを自動生成しません。</p>
    </section>
  );
}
