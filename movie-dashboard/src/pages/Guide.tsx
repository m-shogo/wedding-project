    ...
    <div>
      ...
      <SectionCard title="プロフィールムービーのおすすめ運用" className="mb-6">
        ...
        <StepList items={[
          ...
          "シーン編集で写真スロットを追加する（人物、時期、必要枚数を指定）",
          "写真候補を素材ライブラリに登録する（タイプ「自前写真」、人物タグ・時期タグを設定）",
          "写真計画ページで全体の選定進捗と人物別の不足を確認する",
      ...
      <SectionCard title="各ページの詳細" className="mb-6">
        ...
        <SubHeading>📷 写真計画</SubHeading>
        <p className="text-sm text-navy-600 dark:text-navy-300 mb-2">プロフィールムービーの写真スロットを人物・時期別に管理するページ。</p>
        <BulletList items={[
          "プロフィールムービーのシーンに「写真スロット」を設定する（絵コンテのシーン編集から）",
          "スロットごとに人物（新郎/新婦/ふたり）、時期（幼少期/学生/社会人など）、必要枚数を指定する",
          "候補写真を素材ライブラリに登録し、スロットの候補・選定・不採用に振り分ける",
          "全体の選定進捗、人物別の不足枚数をひと目で確認できる",
          "各スロットカードからシーン詳細ページへリンクできる",
        ]} />
      ...
      {/* ───────── 10. C0 空港ロビー背景 採点基準 ───────── */}
      <SectionCard title="C0 空港ロビー背景 採点基準" className="mb-6">
        <p className="text-sm text-navy-600 dark:text-navy-300 mb-4">
          オープニングムービー冒頭（op-01「出発前の空港・搭乗案内」）で使う、人物なしの空港ロビー背景を採点・採否判断するための基準です。
          既存候補（asset-18, repo内 op_01_narita_boarding_gate_ai.png）は目視確認の結果、人物2名の写り込みが確認され不採用になっています。
        </p>

        <SubHeading>必要条件</SubHeading>
        <BulletList items={[
          "人物なし（最重要）",
          "犬なし",
          "文字なし",
          ...
          "オープニング冒頭に使いやすい",
          "テロップを後から重ねやすい余白がある",
          "Motion Studio素材（搭乗券風演出など）と重ねやすい",
          "色味は上品、白 / ネイビー / ゴールド / やわらかい夕暮れ系寄り",
          "清潔感、上質感がある",
          "雑然としすぎない",
          "画面中央または右寄りに余白が取りやすい",
          "実写ライクすぎてもよいが、広告写真っぽすぎず結婚式ムービーに馴染むこと",
        ]} />

        <SubHeading>避けたいもの</SubHeading>
        <BulletList items={[
          "人物（後ろ姿・シルエットでも不可）",
          "空港スタッフ",
          "混雑",
        ...
        <SubHeading>採点項目（10項目）</SubHeading>
        <StepList items={[
          "人物なしを守れているか（最重要）",
          "文字・ロゴ・看板文字がないか",
          "空港感が伝わるか",
          ...
          "高揚感があるか",
          "テロップ余白があるか",
          "Motion Studio素材と重ねやすいか",
          "C0の次シーン（搭乗券背景 / 雲海）につなぎやすいか",
          "安っぽくないか",
        ]} />

        <SubHeading>採否ルール</SubHeading>
        <BulletList items={[
          "人物あり → 即不採用",
          "読める文字あり → 原則不採用",
          "ロゴあり → 原則不採用",
          "雰囲気は良いが余白不足 → 保留",
          "条件を満たし、C0との接続も良い → 採用候補",
        ]} />

        <Warn>
          人物・文字・ロゴの有無は生成AIの自動判定に頼らず、必ず目視で確認してください。
          asset-status.mdの過去採点（例: op_01が82点）は人物混入を考慮していなかった前例があります。
        </Warn>

        <Tip>
          比較候補プロンプトは Prompt Bank の prompt-13〜prompt-17（方向性A〜E）を参照。
          生成後は各候補を Asset Library で登録し、上記10項目で比較してから1案に絞ります。
        </Tip>
      </SectionCard>

      {/* ───────── 11. localStorageとJSONの注意 ───────── */}
