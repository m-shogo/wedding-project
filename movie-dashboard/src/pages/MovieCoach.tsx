...
type ManualEvidenceState = "learned" | "practiced" | "automated";
...
function ProgressBar({ percent }: { percent: number }) {
  return (
    <div className="h-2 bg-sand-100 dark:bg-navy-700 overflow-hidden">
      <div
        className="h-full bg-navy-700 dark:bg-sand-200 transition-all"
        style={{ width: `${percent}%` }}
      />
    </div>
  );
  ...
  return (
    <div
      className={`border-l-2 pl-3 py-1 ${
        active
          ? "border-navy-800 dark:border-sand-100"
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-sm ${
              active
                ? "font-bold text-navy-800 dark:text-sand-100"
        ...
        <span className="text-[10px] font-mono text-navy-400">{percent}%</span>
      </div>
    </div>
  );
}

function canRecordManualEvidence(
  skill: LearningSkill,
  state: ManualEvidenceState,
  evidence: LearningEvidence[],
): { allowed: boolean; reason: string } {
  if (state === "learned") return { allowed: true, reason: "" };

  if (state === "practiced") {
    return hasEvidenceAtLeast(skill.skillId, "learned", evidence)
      ? { allowed: true, reason: "" }
      : { allowed: false, reason: "先に『理解した』を記録" };
  }

  if (skill.automationPolicy === "never") {
    return { allowed: false, reason: "このSkillは人間判断を残す" };
  }
  if (skill.automationPolicy === "safe_anytime") {
    return { allowed: true, reason: "" };
  }
  if (skill.automationPolicy === "after_practice") {
    return hasEvidenceAtLeast(skill.skillId, "practiced", evidence)
      ? { allowed: true, reason: "" }
      : { allowed: false, reason: "練習Evidenceが必要" };
  }

  return hasEvidenceAtLeast(skill.skillId, "used_in_wedding", evidence)
    ? { allowed: true, reason: "" }
    : { allowed: false, reason: "Wedding実使用Evidenceが必要" };
}
...
export function MovieCoach() {
  ...
  const openingProgress = useMemo(
    () => getMovieCoachProgress("opening", progress),
    [progress],
  );
  const profileCoachProgress = useMemo(
    () => getMovieCoachProgress("profile", progress),
    [progress],
  );
  ...
  const todaySkillIds = today
    ...
    : [];
  const todaySkills = todaySkillIds
    .map(getSkill)
  ...
  function toggleChecklist(outcome: ProductionOutcome, itemId: string) {
    setProgress((current) => {
      const currentItems = new Set(current.outcomeChecklist[outcome.outcomeId] ?? []);
  ...
  function recordManualEvidence(
    ...
    setProgress((current) => {
      const gate = canRecordManualEvidence(skill, state, current.evidence);
      if (!gate.allowed || hasEvidence(skill.skillId, state, outcomeId, current.evidence)) {
        return current;
      }

      const evidence: LearningEvidence = {
        evidenceId: `${skill.skillId}-${state}-${Date.now()}`,
  ...
  function addBookChapter() {
    ...
    const startValue = chapterPageStart.trim() ? Number(chapterPageStart) : null;
    const endValue = chapterPageEnd.trim() ? Number(chapterPageEnd) : null;
    const pageStart = startValue !== null && Number.isFinite(startValue) ? startValue : null;
    const pageEnd = endValue !== null && Number.isFinite(endValue) ? endValue : null;
    ...
    <div>
        ...
        <div>
          <div className="flex items-center justify-between text-xs mb-1">
            ...
            <span className="font-mono text-navy-500">
              {openingProgress.done}/{openingProgress.total} · {openingProgress.percent}%
            </span>
          </div>
          <ProgressBar percent={openingProgress.percent} />
          <p className="text-[11px] text-navy-400 mt-2">
            Remotion正本のOutcomeをWedding実作業で完了した割合
          </p>
        </div>
        <div>
          <div className="flex items-center justify-between text-xs mb-1">
            ...
            <span className="font-mono text-navy-500">
              {profilePhotoStats.selected}/{profilePhotoStats.required} · {profilePhotoStats.percent}%
            </span>
          </div>
      ...
      {today ? (
        <section className="mb-10">
          <div className="flex flex-wrap items-end justify-between gap-4 border-b-2 border-navy-900 dark:border-sand-100 pb-3 mb-5">
            <div>
              ...
              <p className="mt-1 text-xs font-mono text-navy-400">
                {today.authorityLabel} · {today.productionRef}
              </p>
            </div>
            <div className="text-right">
              <p className="text-3xl font-mono font-bold text-navy-900 dark:text-sand-100">
                {todayCompletion?.percent ?? 0}%
              </p>
              <p className="text-xs text-navy-400">
                完成条件 {todayCompletion?.done ?? 0}/{todayCompletion?.total ?? 0}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1.6fr_1fr] gap-6">
            <div className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
                <div>
                  ...
                  <p className="text-sm text-navy-500 dark:text-navy-300 mt-1">
                    練習目安 {today.practiceMinutes}分。終点は練習ではなくWedding本番への適用。
                  </p>
                </div>
                <div>
                  ...
                  {todayBookChapters.length > 0 ? (
                    <div className="mt-1 space-y-1">
                      {todayBookChapters.map(({ skillId, chapter }) => (
                        <p
                          key={`${skillId}-${chapter!.chapterId}`}
                          className="text-sm text-navy-700 dark:text-navy-200"
                        >
                          {chapter!.title}
                    ...
                    <p className="mt-1 text-sm text-amber-700 dark:text-amber-300">
                      Book Manifest未接続。下で章・ページとSkillを1回登録すると自動表示します。
                    </p>
                  )}
                </div>
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {today.conceptSkillIds.map((skillId) => {
                      ...
                      return skill ? (
                        <span
                          key={skillId}
                          className="text-xs border-b border-navy-300 pb-0.5 text-navy-700 dark:text-navy-200"
                        >
                          {skill.label}
                        </span>
                      ) : null;
                ...
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {today.davinciSkillIds.map((skillId) => {
                      const skill = getSkill(skillId);
                      return skill ? (
                        <span key={skillId} className="text-xs font-mono text-navy-700 dark:text-sand-200">
                          {skill.davinciPage} / {skill.label}
                        </span>
                      ) : null;
                    })}
                  </div>
              ...
              <div className="border-t border-sand-200 dark:border-navy-600 pt-4">
                ...
                <p className="mt-1 text-xs text-navy-400">
                  全条件を満たすと、このOutcomeで使ったSkillへWedding実使用Evidenceを自動記録します。
                </p>
                <div className="mt-3 space-y-2">
                  {today.checklist.map((item) => {
                    ...
                    return (
                      <label
                        key={item.itemId}
                        className="flex items-center gap-3 text-sm text-navy-700 dark:text-navy-200 cursor-pointer"
                      >
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => toggleChecklist(today, item.itemId)}
                          className="h-4 w-4"
                        />
            ...
            <aside className="border-l border-sand-200 dark:border-navy-600 pl-5">
              ...
              <div className="space-y-2">
                {productionOutcomes
                  .filter((outcome) => outcome.movieId === "opening")
                  .map((outcome) => (
                    <OutcomeRail
                      key={outcome.outcomeId}
                      outcome={outcome}
                      active={outcome.outcomeId === today.outcomeId}
                      percent={getOutcomeCompletion(outcome, progress.outcomeChecklist).percent}
                    />
                  ))}
              </div>
              <Link
                to="/profile-planner"
                className="inline-block mt-5 text-xs text-navy-600 dark:text-navy-300 underline"
              >
                Profile写真計画を見る →
              </Link>
            </aside>
        ...
        <div className="mb-10 p-6 border border-emerald-200 bg-emerald-50 dark:bg-emerald-900/20 dark:border-emerald-800">
          ...
          <p className="text-sm text-emerald-700 dark:text-emerald-300 mt-1">
            次はReview結果から新しいProduction Outcomeを追加します。
          </p>
        </div>
      )}
      ...
      {today && (
        <SectionCard title="Learn → Try → Apply → Evidence" className="mb-8">
          <p className="text-sm text-navy-500 dark:text-navy-300 mb-4">
            「本番で使った」はOutcome完成時に自動記録。「自力で使える」は同じSkillを別Wedding Outcomeで2回使うと自動昇格します。
          </p>
          <div className="space-y-4">
            {todaySkills.map((skill) => {
              const currentState = getSkillState(skill.skillId, progress.evidence);
              const learnedRecorded = hasEvidence(
                skill.skillId,
                "learned",
                today.outcomeId,
                progress.evidence,
              );
              const practicedRecorded = hasEvidence(
                skill.skillId,
                "practiced",
                today.outcomeId,
                progress.evidence,
              );
              const automatedRecorded = hasEvidence(
                skill.skillId,
                "automated",
                today.outcomeId,
                progress.evidence,
              );
              const practiceGate = canRecordManualEvidence(skill, "practiced", progress.evidence);
              const automateGate = canRecordManualEvidence(skill, "automated", progress.evidence);

              return (
                <div
                  key={skill.skillId}
                  className="border-t border-sand-200 dark:border-navy-600 pt-4 first:border-t-0 first:pt-0"
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="max-w-2xl">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-[10px] tracking-widest font-semibold text-navy-400">
                          {skill.kind === "concept" ? "CONCEPT" : `DAVINCI / ${skill.davinciPage}`}
                        </span>
                        <span className={`px-2 py-0.5 text-[11px] ${stateTone[currentState]}`}>
                          {learningStateLabel[currentState]}
                        </span>
                      </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 items-center">
                    <button
                      type="button"
                      disabled={learnedRecorded}
                      onClick={() => recordManualEvidence(skill, "learned", today.outcomeId)}
                    ...
                    >
                      {learnedRecorded ? "✓ " : ""}理解した
                    </button>
                    <button
                      type="button"
                      disabled={practicedRecorded || !practiceGate.allowed}
                      title={practiceGate.reason}
                      onClick={() => recordManualEvidence(skill, "practiced", today.outcomeId)}
                      className={`px-2.5 py-1.5 text-xs border ${
                        practicedRecorded
                          ? "border-emerald-200 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:border-emerald-800 dark:text-emerald-300"
                          : practiceGate.allowed
                            ? "border-sand-200 text-navy-600 hover:border-navy-500 dark:border-navy-600 dark:text-navy-200"
                            : "border-sand-100 text-navy-300 cursor-not-allowed dark:border-navy-700 dark:text-navy-600"
                      }`}
                    >
                      {practicedRecorded ? "✓ " : ""}練習した
                    </button>
                    <span className="px-2.5 py-1.5 text-xs border border-sand-100 text-navy-400 dark:border-navy-700">
                      本番使用 = Outcome完成で自動
                    </span>
                    <button
                      type="button"
                      disabled={automatedRecorded || !automateGate.allowed}
                      title={automateGate.reason}
                      onClick={() => recordManualEvidence(skill, "automated", today.outcomeId)}
                      className={`px-2.5 py-1.5 text-xs border ${
                        automatedRecorded
                          ? "border-violet-200 bg-violet-50 text-violet-700 dark:bg-violet-900/20 dark:border-violet-800 dark:text-violet-300"
                          : automateGate.allowed
                            ? "border-sand-200 text-navy-600 hover:border-navy-500 dark:border-navy-600 dark:text-navy-200"
                            : "border-sand-100 text-navy-300 cursor-not-allowed dark:border-navy-700 dark:text-navy-600"
                      }`}
                    >
                      {automatedRecorded ? "✓ " : ""}自動化を記録
                    </button>
                    {!practiceGate.allowed && (
                      <span className="text-[10px] text-navy-400">{practiceGate.reason}</span>
                    )}
                  </div>
                </div>
              );
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-8">
          ...
          <div className="mt-4 space-y-3 max-h-[420px] overflow-auto pr-1">
            {filteredSkills.map((skill) => {
              ...
              return (
                <div
                  key={skill.skillId}
                  className="border-b border-sand-100 dark:border-navy-700 pb-3 last:border-0"
                >
                  <div className="flex items-center justify-between gap-3">
                    <p className="font-semibold text-sm text-navy-800 dark:text-sand-100">{skill.label}</p>
                    <span className={`px-2 py-0.5 text-[10px] ${stateTone[state]}`}>
                      {learningStateLabel[state]}
                    </span>
                  </div>
                  <p className="text-xs text-navy-500 dark:text-navy-300 mt-1">{skill.summary}</p>
                  {skill.kind === "davinci" && (
                    <p className="text-[11px] font-mono text-navy-400 mt-1">
                      {skill.davinciPage} · {(skill.features ?? []).join(" / ")}
                    </p>
                  )}
                </div>
              );
        ...
        <SectionCard title="Good / Bad — 既存Recipeを教材化">
          ...
          <Link to="/clips" className="inline-block mt-4 text-xs text-navy-600 dark:text-navy-300 underline">
            既存クリップ素材集 / Recipeを見る →
          </Link>
        </SectionCard>
      </div>
      ...
      <SectionCard title="購入済みDaVinci本 — Book Manifest" className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
          <label className="text-xs text-navy-500 dark:text-navy-300">
            書籍タイトル
            <input
              value={progress.book.title}
              onChange={(event) => updateBook("title", event.target.value)}
              className="mt-1 w-full px-3 py-2 border border-sand-200 bg-white text-sm text-navy-800 dark:bg-navy-800 dark:border-navy-600 dark:text-sand-100"
            />
          </label>
          <label className="text-xs text-navy-500 dark:text-navy-300">
            版 / edition
            <input
              value={progress.book.edition}
          ...
          <label className="text-xs text-navy-500 dark:text-navy-300">
            対応DaVinci Version
            <input
              value={progress.book.davinciVersion}
              onChange={(event) => updateBook("davinciVersion", event.target.value)}
              className="mt-1 w-full px-3 py-2 border border-sand-200 bg-white text-sm text-navy-800 dark:bg-navy-800 dark:border-navy-600 dark:text-sand-100"
            />
          </label>
        </div>
        ...
        <div className="border-t border-sand-200 dark:border-navy-600 pt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1.4fr_0.7fr_0.7fr_1.2fr_auto] gap-2 items-end">
            <label className="text-xs text-navy-500 dark:text-navy-300">
              章名
              <input value={chapterTitle} onChange={(event) => setChapterTitle(event.target.value)} className="mt-1 w-full px-2 py-2 border border-sand-200 bg-white text-sm dark:bg-navy-800 dark:border-navy-600 dark:text-sand-100" />
            </label>
            <label className="text-xs text-navy-500 dark:text-navy-300">
              開始P
              <input type="number" min="1" value={chapterPageStart} onChange={(event) => setChapterPageStart(event.target.value)} className="mt-1 w-full px-2 py-2 border border-sand-200 bg-white text-sm dark:bg-navy-800 dark:border-navy-600 dark:text-sand-100" />
            </label>
            <label className="text-xs text-navy-500 dark:text-navy-300">
              終了P
              <input type="number" min="1" value={chapterPageEnd} onChange={(event) => setChapterPageEnd(event.target.value)} className="mt-1 w-full px-2 py-2 border border-sand-200 bg-white text-sm dark:bg-navy-800 dark:border-navy-600 dark:text-sand-100" />
            </label>
            <label className="text-xs text-navy-500 dark:text-navy-300">
              Skill
              <select value={chapterSkillId} onChange={(event) => setChapterSkillId(event.target.value)} className="mt-1 w-full px-2 py-2 border border-sand-200 bg-white text-sm dark:bg-navy-800 dark:border-navy-600 dark:text-sand-100">
                {learningSkills.map((skill) => (
                  <option key={skill.skillId} value={skill.skillId}>{skill.label}</option>
                ))}
              </select>
            </label>
            <button type="button" onClick={addBookChapter} className="px-3 py-2 bg-navy-800 text-white text-xs dark:bg-sand-100 dark:text-navy-900">
              追加
            </button>
          </div>
        </div>
        ...
        {progress.book.chapters.length > 0 && (
          <div className="mt-5 overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-navy-400 border-b border-sand-200 dark:border-navy-600">
                  <th className="py-2">章</th>
                  <th>ページ</th>
                  <th>Skill</th>
                </tr>
              </thead>
              <tbody>
                {progress.book.chapters.map((chapter) => (
                  <tr key={chapter.chapterId} className="border-b border-sand-100 dark:border-navy-700">
                    ...
                    <td className="font-mono text-xs text-navy-500">
                      {chapter.pageStart ?? "?"}–{chapter.pageEnd ?? "?"}
                    </td>
                    <td className="text-xs text-navy-500">
                      {chapter.skillIds
                        .map((skillId) => getSkill(skillId)?.label ?? skillId)
                        .join(", ")}
                    </td>
                  </tr>
                ))}
      ...
      <SectionCard title="Profile Movieへつなぐ">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div>
            <p className="text-[11px] tracking-widest text-navy-400 font-semibold">PHOTO PLAN</p>
            <p className="mt-1 text-2xl font-mono font-bold text-navy-900 dark:text-sand-100">{profilePhotoStats.percent}%</p>
            <p className="text-xs text-navy-500 mt-1">{profilePhotoStats.selected}/{profilePhotoStats.required}枚を選定済み</p>
          </div>
          <div>
            <p className="text-[11px] tracking-widest text-navy-400 font-semibold">COACH OUTCOME</p>
            <p className="mt-1 text-2xl font-mono font-bold text-navy-900 dark:text-sand-100">{profileCoachProgress.percent}%</p>
            <p className="text-xs text-navy-500 mt-1">まず写真選定 → Story → 長尺Pacingへ進む</p>
          </div>
          <div>
            <p className="text-[11px] tracking-widest text-navy-400 font-semibold">PRINCIPLE</p>
            <p className="mt-1 text-sm font-semibold text-navy-800 dark:text-sand-100">派手な演出よりStoryが伝わること</p>
            <Link to="/profile-planner" className="inline-block mt-2 text-xs underline text-navy-600 dark:text-navy-300">
              Profile Plannerへ →
            </Link>
          </div>
        </div>
