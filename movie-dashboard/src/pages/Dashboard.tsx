    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
        ...
        <StatCard
          icon="⏱"
          label="尺"
          value={`${stats.totalDurationSec}秒 / ${stats.targetDurationSec}秒`}
        />
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        ...
        <StatCard
          icon="🔗"
          label="未紐付素材"
          value={stats.unlinkedAssets}
          accent="text-amber-600"
        />
        <StatCard
          icon="📝"
          label="未紐付プロンプト"
          value={stats.unlinkedPrompts}
          accent="text-amber-600"
        />
        <StatCard
          icon="🤖"
          label="AI動画予定"
          value={stats.aiVideoPlanned}
          accent="text-indigo-600"
        />
        <StatCard
          icon="✂"
          label="CapCut準備済"
          value={stats.capcutReady}
            ...
            {stats.urgentTasks.map((t) => (
              <li key={t.taskId} className="flex items-start gap-3">
                ...
                <Badge
                  ...
                  colorClass={taskStatusColor[t.status]}
                />
        ...
        <SectionCard title="詰まっているシーン" className="mb-8">
          <div className="space-y-3">
            {stats.blockedScenes.map((s) => (
              ...
              >
                <Badge
                  label={sceneStatusLabel[s.status]}
                  colorClass={sceneStatusColor[s.status]}
                />
                <div className="min-w-0">
                  <p className="text-sm font-medium text-navy-800">
                    {s.title}
                  ...
                  <p className="text-xs text-navy-400 font-mono">
                    {s.sceneId} &middot; {s.durationSec}秒
                  </p>
                </div>
              </div>
            ))}
      ...
      <SectionCard title="ムービー別進捗" className="mb-8">
        <div className="space-y-4">
          {data.movies.map((movie) => {
            const mScenes = data.scenes.filter(
              ...
              <div key={movie.movieId} className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      label={movieStatusLabel[movie.status]}
                      colorClass={movieStatusColor[movie.status]}
                    />
                  </div>
                  <span className="text-xs text-navy-500">
                    {mDone}/{mTotal} シーン完了 ({pct}%)
                  </span>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {movieScenes.map((scene) => (
            ...
            >
              ...
              <p className="text-xs text-navy-500 mb-2 line-clamp-2">
                {scene.purpose}
              </p>
