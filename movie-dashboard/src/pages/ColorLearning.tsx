...
import {
  colorDecisionRules,
  colorLearningStages,
  colorWeddingExercises,
  type ColorStageId,
} from "../data/colorLearning";

export function ColorLearning() {
  const [selectedStageId, setSelectedStageId] = useState<ColorStageId>("original");
  const [selectedExerciseId, setSelectedExerciseId] = useState(colorWeddingExercises[0]?.exerciseId ?? "");

  const selectedStage = useMemo(
    () => colorLearningStages.find((stage) => stage.stageId === selectedStageId) ?? colorLearningStages[0],
    [selectedStageId],
  );
  const selectedExercise = useMemo(
    () => colorWeddingExercises.find((exercise) => exercise.exerciseId === selectedExerciseId) ?? colorWeddingExercises[0],
    [selectedExerciseId],
  );
  ...
  return (
    <div>
      <Header
        ...
        description="LUTから始めず、写真・動画間の違和感を順番に減らしてWedding Movie全体を自然につなぐ"
      />

      <section className="mb-9 border-y border-sand-200 dark:border-navy-600 py-5">
        <p className="text-[10px] tracking-[0.2em] font-semibold text-navy-400">COLOR PRINCIPLE</p>
        <h2 className="mt-1 text-xl font-bold text-navy-900 dark:text-sand-100">「綺麗にする」より「前後で違和感を減らす」</h2>
        <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-3">
          {colorDecisionRules.map((rule, index) => (
      ...
      <section className="mb-10">
        <div className="border-b-2 border-navy-900 dark:border-sand-100 pb-3 mb-4">
          <p className="text-[10px] tracking-[0.2em] font-semibold text-navy-400">ORIGINAL → FINAL</p>
          <h2 className="mt-1 text-xl font-bold text-navy-900 dark:text-sand-100">1回に1つだけ判断する</h2>
        </div>

        <div className="overflow-x-auto pb-2">
          <div className="flex min-w-max border-y border-sand-200 dark:border-navy-600">
            {colorLearningStages.map((stage) => {
      ...
      <section className="mb-10">
        <div className="border-b-2 border-navy-900 dark:border-sand-100 pb-3 mb-4">
          ...
          <h2 className="mt-1 text-xl font-bold text-navy-900 dark:text-sand-100">本番素材でShot Matchを練習する</h2>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[0.7fr_1.4fr] gap-7">
          <div className="border-y border-sand-200 dark:border-navy-600 divide-y divide-sand-100 dark:divide-navy-700">
            {colorWeddingExercises.map((exercise) => {
      ...
      <section className="mb-8 border-y border-sand-200 dark:border-navy-600 py-5">
        <p className="text-[10px] tracking-[0.2em] font-semibold text-navy-400">BEFORE / AFTER PROTOCOL</p>
        <h2 className="mt-1 text-xl font-bold text-navy-900 dark:text-sand-100">1枚のBefore/Afterだけで判断しない</h2>
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-6 text-sm text-navy-700 dark:text-navy-200">
          <div><span className="font-mono text-navy-400">01</span><p className="mt-1">OriginalとFinalをON/OFFして、補正が強すぎないか確認。</p></div>
          <div><span className="font-mono text-navy-400">02</span><p className="mt-1">前後shotと並べ、切替時の違和感を見る。</p></div>
          <div><span className="font-mono text-navy-400">03</span><p className="mt-1">Timelineを止めずに再生し、色よりStoryへ目が行くか確認。</p></div>
        </div>
      </section>

      <div className="flex flex-wrap gap-4 text-xs">
        <Link to="/movie-coach" className="border-b border-navy-300 text-navy-600 dark:text-navy-300">← Movie Coach</Link>
        <Link to="/movie-coach/dictionary?q=色" className="border-b border-navy-300 text-navy-600 dark:text-navy-300">逆引き辞典で色を調べる →</Link>
        <Link to="/movie-coach/review" className="border-b border-navy-300 text-navy-600 dark:text-navy-300">Color FindingをReviewする →</Link>
      </div>
    </div>
  );
}
