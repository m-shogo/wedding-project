# WEDDING PASSPORT — Fixed-art Vector Study QA / 2026-08-24

State: `THREE_CLEANROOM_FIXED_ART_STUDIES_CREATED / A+B_ADVANCE_TO_FIGMA_ASSEMBLY / C_HOLD / NO_PRODUCTION_PROMOTION`

## Authority and scope

- start main: `1a73cff5d85504a24780c6b5393e5999d68a9f6a`
- Current visual authority: `docs/automation/non-rurubu-figma-quality-current.md`
- Hybrid authoring authority: `docs/design-learning/AI-FIGMA-HYBRID-AUTHORING-POLICY.md`
- source art-direction brief: `PROFESSIONAL-CLEANROOM-VNEXT-ART-DIRECTION-BRIEF-2026-08-24.md`
- Figma file: `UbK8KmuWJcDeGScsN49Uor`
- retained Current front/back: `181:52 / FIELD JOURNAL`, `181:80 / RETURN NOTE`
- exact Drive authority: `1LnGb9tq_Vswe-GKO6UxfvKMAZuShEaaw / 01_パスポート風_メニュー・ドリンク・座席表`

No retained production visual node, layout group, decorative vector, crop or generated asset was copied into these studies. The studies start from a blank 1480×2100 coordinate system and contain no authoritative copy.

## New professional design learning used

The research step was tightened with current high-quality primary design sources:

- Pentagram Books: book design must orchestrate typography, layout, printing and binding into a storytelling experience; the printed book has authority and endurance beyond a one-frame poster.
- Pentagram Travel + Leisure: strong travel editorial design opens layouts, foregrounds imagery/visual atmosphere, and keeps information hierarchy clear rather than simulating density with containers.
- Pentagram United Airlines: one strong travel gesture can carry emotional flight energy when it is used boldly rather than multiplied into travel clichés.
- Pentagram Impala: everyday travel vernacular can generate excitement, but literal ticket/signage language must remain appropriate to the artifact rather than becoming fake administrative UI.

Sources:
- https://www.pentagram.com/book-design
- https://www.pentagram.com/work/travel-leisure
- https://www.pentagram.com/work/united-airlines/story
- https://www.pentagram.com/work/impala/story

Transferable conclusion for this item: **increase travel energy through one dominant spatial gesture and publication rhythm while protecting the cover's keepsake-booklet reading.** Do not solve anticipation by adding more planes, stamps, badges or fake transport fields.

## Three clean-room fixed-art studies

All three are fixed-art/support studies only. Final names, date, place, headlines and emotional copy must remain native Figma text during assembly.

### A — Departure Window

File:
- `studies/vnext-2026-08-24/departure-window-fixed-art.svg`

Visual strategy:
- large curved opening occupying the upper/right field;
- one bright sun anchor;
- coral + lagoon movements enter the lower field;
- narrow left stitch-like vertical rhythm is intentionally physical/supportive rather than a fake passport credential.

Thumbnail critique:
- strongest immediate sense of departure, air and forward movement;
- materially different from FIELD JOURNAL;
- sufficiently broad native-text-safe cream field remains available;
- primary risk: if native headline/date are too poster-like, the object may lose booklet specificity.

Decision: `ADVANCE_TO_FIGMA_ASSEMBLY`.

### B — Island Field Guide

File:
- `studies/vnext-2026-08-24/island-field-guide-fixed-art.svg`

Visual strategy:
- coastal/wind rhythm expressed as three asymmetric line weights;
- dark publication header field;
- warm coral/mango coastal light entering from the upper right;
- lower factual-anchor fields intentionally remain abstract and contain no final wording.

Thumbnail critique:
- strongest Hawaii/destination atmosphere without tourist icon clusters;
- most editorial/collectible of the three;
- avoids literal map labels, flight numbers, tickets and UI;
- primary risk: can become quiet/gallery-like unless native Japanese type introduces enough celebration scale and warmth.

Decision: `ADVANCE_TO_FIGMA_ASSEMBLY`.

### C — Sunlit Departure Folio

File:
- `studies/vnext-2026-08-24/sunlit-departure-folio-fixed-art.svg`

Visual strategy:
- large coral/mango top sweep;
- deep-ocean lower movement with lagoon inset;
- broad open cream lane for native editorial copy.

Thumbnail critique:
- strongest raw celebration energy;
- strong scale change and off-frame movement;
- however, the large multicolor sweep/fold vocabulary risks converging with family patterns already corrected elsewhere in the non-Rurubu suite;
- it currently reads closer to a premium event poster/folio than a uniquely passport-like keepsake.

Decision: `HOLD / DO_NOT_MATURE_FIRST`.

This is not a permanent rejection. It should only return if A/B fail to produce a mature cover or if a later assembly can prove materially stronger booklet specificity.

## Hybrid authoring split

For the next Figma assembly:

- native Figma text: all Japanese/Latin reader-facing copy, names, date, place and placeholders;
- SVG fixed art: A/B study geometry may be imported only as a fixed-art support layer or rebuilt as a sane editable vector group if import/editability is better;
- replaceable photography: none for this cover experiment;
- generated raster: none in this study; do not generate merely to fill space;
- no final copy is baked into these SVGs.

## QA and adoption boundary

The local SVGs were rendered at approximately 370×525 thumbnail scale before Git write and inspected as a three-way set. This is only fixed-art thumbnail evidence; it is **not** a sellable-cover PASS.

Required before any promotion:

1. assemble A and B independently in Figma on blank frames;
2. add native Japanese/Latin copy without referencing retained Current composition;
3. compare A vs B at 500px whole-item scale;
4. mature the winner(s) at reading and 1480×2100 actual size;
5. stress long names/copy and verify native text auto-height;
6. verify any claimed paper/binding/edge cue actually behaves as a physical or editorial role;
7. only after maturation compare against retained `181:52 / 181:80`;
8. promote only if the new family clearly improves travel anticipation without becoming a poster or fake travel administration.

## Current tool limitation

Read-only Figma screenshot and exact Drive authority readback are available. The current connector does not expose an approved readable `figma-use` skill resource, while the Figma write action explicitly requires that guidance before use. Therefore this run did **not** mutate Figma or claim placement/three-scale verification.

Per the repeated-failure rule, the run did not keep retrying the same blocked write path. It method-switched to producing the clean-room fixed-art inputs needed by the next safe Figma assembly step.

## Result

- new fixed-art studies: `3`
- candidates advanced: `A Departure Window`, `B Island Field Guide`
- candidate held: `C Sunlit Departure Folio`
- Figma write: `0`
- Drive write: `0`
- generated raster: `0`
- production promotion: `0`

The retained FIELD JOURNAL / RETURN NOTE remains Current until a fully assembled clean-room candidate clearly wins.
