...
`V6_LIVE_FIGMA_IN_PROGRESS / OUTER_IQ_PREFERRED / PROFILE_QA_IP_PREFERRED / STORY_CHRONOLOGY_IO_PREFERRED / MEMORY_SPOTS_IH_PREFERRED / GOURMET_CAFE_IN_PREFERRED / ONE_DAY_PLAN_IM_PREFERRED / NATIVE_VARIABLE_TEXT_PRESERVED / REPLACEABLE_PHOTOS_PRESERVED / IQ_IP_IO_IH_IN_IM_THREE_SCALE_DUMMY_DESIGN_QA_PASS / V7_HOLD / NOT_PRINT_READY`
...
Note: GitHub status had temporarily lagged live Figma and still named IK even though live IP `2096:2` was already preferred. This file now follows the live authority.
...
- ID `2051:2` → `ROLLBACK_HIDDEN / V6_OUTER_ID_HN_BACK_PHOTOLED_FRONT_2026_08_21`, x=`277400`, hidden, not deleted.
...
- IO `2095:18`: taller chronology route photo and tightened terminal sequence; three-scale QA PASS.
...
Drive V6 root reverified before IQ work:
...
`V6 IQ + IP + IO + IH + IN + IM = VERIFIED_LOCAL_DUMMY_DESIGN_STUDIES / PHOTO_LED_TRAVEL_GUIDE_GRAMMAR / NATIVE_TEXT_EDITABILITY_PRESERVED / REPLACEABLE_PHOTOS_PRESERVED / ROLLBACK_SAFE / V7_HOLD / NOT_PRINT_READY`.

## Next highest-value work

1. Continue V6, not V7.
2. Re-read live Figma, Drive and latest GitHub main before every write.
3. Re-run the common-scale preferred-set comparison using `IQ + IP + IO + IH + IN + IM` and select the next weakest page.
4. Do not keep polishing IQ merely because it is newest.
5. Keep ID, IK, IL, IE, IG, IJ, HU, IF, GY, HS, HT and HC hidden as rollback/comparison evidence.
