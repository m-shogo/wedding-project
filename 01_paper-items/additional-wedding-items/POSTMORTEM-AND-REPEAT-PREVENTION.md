...
Date: 2026-08-01
Scope: 完成済み4種を除く追加ウェディングアイテム17種
...
- 対象pathが完成済み4種配下なら操作を中止し、追加17種の先頭未完了へ戻る。
...
- 毎runで最新mainを読む。
- `MASTER-LIST.md`と`CURRENT-STATUS.md`もCurrent authorityとして読む。
...
### 強制対策
- 本スケジュールはFigma全面禁止。
- Figmaが復旧・課金済みでも追加17種の自動制作からは触らない。
- Figma作業は別の明示指示・別Currentでのみ実行する。
- 自動制作ではFigma課金後にすぐ配置できる`FIGMA-PLACEMENT-BRIEF.md`まで準備する。
...
- 不採用を無理に採用しない。
- Phase Cの条件付きアイテムは勝手に事実を作らず、テンプレ準備までを正確に記録する。

## 14. Figma課金後の準備不足で再設計が発生する危険

### 原因
- 素材だけ作り、サイズ・可変テキスト・semantic node・配置指示を残さないと、Figma開始後に検討をやり直す。

### 強制対策
- 各アイテムに`SPEC.md`、`ASSET-QUEUE.md`、`DRIVE-REGISTER.md`、`QA.md`、`FIGMA-PLACEMENT-BRIEF.md`を準備する。
- 印刷サイズ、塗り足し、安全域、可変テキスト、semantic node name案を記録する。
- QRコード、ゲスト名、卓名、時刻、長文を画像に焼き込まない。
- 不明情報は`TBD`またはダミーと明示し、事実として確定しない。
...
速く大量に作ることより、同じ失敗を繰り返さず、採用物を確実にDriveとGitへ残し、次runとFigma課金後の作業が迷わず続けられることを優先する。
