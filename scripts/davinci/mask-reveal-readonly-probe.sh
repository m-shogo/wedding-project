...
# Read-only probe for the Mask Reveal vertical slice.
# This script does NOT launch Resolve, install an MCP, edit config, or write project files.
...
if command -v npm >/dev/null 2>&1; then
  if npm ls -g --depth=0 2>/dev/null | grep -q "davinci-resolve-mcp"; then
    npm_global_hint="YES"
  fi
fi
...
printf 'davinciMcpNpmGlobalHint=%s\n' "$npm_global_hint"
printf 'davinciMcpConfiguredHint=%s\n' "$mcp_config_hint"
printf '%s\n' "authoritativeVersionSource=Resolve MCP resolve_control(get_version) after connection"
printf '%s\n' "mutationsPerformed=NO"
printf '%s\n' "next=Use the local execution runbook; do not promote Actual/TESTED from this probe alone."
