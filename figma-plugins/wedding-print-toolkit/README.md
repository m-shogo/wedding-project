...
航空券などのトップレベルテンプレFrame / Component / Instanceを1つ選択し、`GUEST_NAME` layerへ短名〜長名の4パターンを流したcloneを右側へ生成する。

- 元デザインは変更しない
- cloneへ `weddingStressTest=true` のpluginDataを付ける
- 日本語のやや長い名前と長い英字名を混ぜる
- `GUEST_NAME` / `GUEST NAME` / `{{GUEST_NAME}}` / `ゲスト名` を対象にする
- Mixed fontやMissing fontは安全停止

目的は「本番データを流す前に、名前で崩れるテンプレを発見する」こと。
...
- Stress Testが元Frameを変更せず4cloneだけ生成する
- 長い名前を入れたcloneを目視してoverflow/折返しを評価できる
- 既存Designを破壊しない
- 実際の4本番ファイルの少なくとも1つでQA成功

## 次の候補
1. Guest CSV Populate
2. Print QA Overlay（safe area / bleed / crop guide）
3. Photo Placeholder Batch Swap
4. Batch Export Helper
5. Asset-Frozen marker / Current status helper
