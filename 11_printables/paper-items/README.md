...
- **このプロジェクト内の通常作業は包括承認済みとして扱い、生成・調査・Drive保存・Git更新・Figma反映・Plugin試作・QAのたびに個別確認を求めない。**
- 確認が必要なのは、削除・公開範囲変更・課金/購入・外部への送信・本番データ破壊・取り返しのつきにくい変更など、明確に不可逆または高リスクな操作のみ。
